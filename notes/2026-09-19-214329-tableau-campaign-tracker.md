...
| 2.6c | `PublishedRow` -> `Published`, `PublishedRowAt` -> `PublishedAt`, `AnalysisPathRow` -> `AnalysisPath`, `*_rowE` -> `*_pathE`, instance values `<inst>_<model>_published` and `<inst>_<model>_path`, then the prose (868 uses of "row" in 26 files) as an audited comment pass. Owner decided 2026-09-20: `Published`, all layers. Plan: `notes/2026-09-20-111257-published-row-rename-plan.md`. Runs after 2.6a and 2.6b | decided, open |
| 2.6d | Reminder owed to the owner when every other task is done: four files in the forward closure of `psl211_endpoints.v` still hold economic words (`security/pgg_collusion_bound.v`, `protocol/pgg_algebra_syntax.v`, `instances/psl211/psl211_exec.v`, `instances/psl211/psl211_profile.v`); editing them means recompiling the endpoint file (900 s, 17 GB), which is the owner's call | open |
