...
| 2.6c | `PublishedRow` -> `Published`, `PublishedRowAt` -> `PublishedAt`, `AnalysisPathRow` -> `AnalysisPath`, `*_rowE` -> `*_pathE`, instance values `<inst>_<model>_published` and `<inst>_<model>_path`, then the prose (868 uses of "row" in 26 files) as an audited comment pass. Owner decided 2026-09-20: `Published`, all layers. Plan: `notes/2026-09-20-111257-published-row-rename-plan.md`. Runs after 2.6a and 2.6b | FIRST COMMIT DONE (layers A to E: 110 identifiers, 710 occurrences, 16 files; map, checker, Locate probe, apply report, closure log and the re-check of 16 recorded `Fail` commands in `notes/probes/2026-09-20-published-rename/`; names list `notes/2026-09-20-142813-renamed-identifiers-published.md`). SECOND COMMIT open: the prose, by `prose_sheet.md` (program / path / unchanged) and `check_prose.py` (position-by-position word check); 1175 comment uses of the word in the tree, about a third of them rows of matrices, tables and traces that stay |
