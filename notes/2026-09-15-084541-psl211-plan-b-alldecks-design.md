...
lemma `uniform_pair_indep_of_fibers`. Outside the flow: the fixed-dealer colour result of Plan A (stays in
...
- Files: `psl211_alldecks.v` (layout, validity, the per-cut fiber counting C5,
  ...
  `reconstruct/design_privacy.v` gains the generic bridge `uniform_pair_indep_of_fibers`
  (section 4, C3).
