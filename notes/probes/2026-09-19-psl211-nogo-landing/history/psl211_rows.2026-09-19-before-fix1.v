...
(* instances/psl211/psl211_spectral_nogo.v restates that field as             *)
...
(* strictly below 1/1320, psl211_alldecks_no_spectral_cert, so a row over this*)
(* model that publishes its certificate's own bound publishes at least 1/660. *)
(* In particular no certificate carries the instance's own marginal bound,    *)
(* whose epsilon is zero, psl211_alldecks_no_spectral_cert0. The field fails  *)
(* at two deck descriptions of one chirality as well,                         *)
(* psl211_alldecks_constancy_false_blockline, so it asks for more than the    *)
(* secret-independence this row claims and its failure is not leakage. What is*)
(* excluded is a range of epsilon and not the arm. Trace secrecy is not a     *)
(* field of an AnalysisPathRow, so no trace row is written and the executed   *)
(* content reader of psl211_models.v is what the facade carries instead.      *)
