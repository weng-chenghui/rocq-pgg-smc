...
STAGED_INST = os.path.join(HERE, "staged", "instances", "kim2025")
STAGED_DIR = os.path.join(STAGED_INST, "tableau")
PHASES = ["algebraic", "executable", "observed", "sampled",
          "analysis_bridged", "checks"]
STAGED_PHASE = [os.path.join(STAGED_DIR, "five_card_tableau_%s.v" % p)
                for p in PHASES]
STAGED_PROX = os.path.join(STAGED_INST, "five_card_proximity.v")
...
NEW_NAMES = {"five_card_algebraic_start",
...
# The declarations that are law and distance mathematics and stay in
# instances/kim2025/five_card_proximity.v.  Nothing in the tableau directory
# but the proximity certificate's last field reads any of them.
STAYING = {("Lemma", "five_card_uniform_pairE"),
           ("Lemma", "five_card_reading_secretE"),
           ("Lemma", "five_card_arg_cut_prodE"),
...
INTENDED_DOCSTRING_DIFF = {
    "five_card_target": (
        "'The prefix above names the same one' becomes 'The prefix "
        "five_card_committed names the same one', and 'held together by "
        "five_card_realises_expected below' loses the 'below': the prefix "
        "and that lemma are in the Observed file and this record is in the "
        "Algebraic one"),
    "five_card_row_uniform_tableau": (
        "'the prefix above, the uniform rotation model' becomes 'the prefix "
        "five_card_committed, the uniform rotation model': the prefix is in "
        "the Observed file, while the witness named next is still above"),
    "five_card_row_repeated_tableau": (
        "'the prefix above and the seven-cut model' becomes 'the prefix "
        "five_card_committed and the seven-cut model': the prefix is in the "
        "Observed file and this value in the Sampled one"),
    "five_card_row_biased_levelE": (
        "'the rejected ascription that follows' becomes 'the rejected "
        "ascription of five_card_tableau_checks.v': the equation is at "
        "Sampled and the rejection is in the checks file"),
    "five_card_row_s5_family": (
        "'The two models above are typed' becomes 'The two models named at "
        "Sampled are typed': the two models are in the Sampled file and "
        "this rejection in the checks file"),
    "kim_biased_proximity_cert": (
        "'kim_biased_proximity_close of this file' becomes "
        "'kim_biased_proximity_close of five_card_proximity.v': the "
        "distance stays in the reduced file and the certificate moves to "
        "the AnalysisBridged one"),
}
...
# The one intended difference between two rejection messages.  The eleven
# recorded Fails come from two production files whose import lists differ,
# and they land in one checks file, so one of the two environments cannot be
# reproduced.  five_card_rows.v does not import s5_exec and
# five_card_proximity.v does; the checks file follows the first, because no
# name of s5_exec is used in it and an import has to be earned.  The
# difference is the printer qualifying one constant, not a different
# constant: production prints s5_rand_observed and the staged side
# s5_exec.s5_rand_observed, and the rest of the message is byte-identical.
INTENDED_FAIL_DIFF = {
    "kim_biased_cert_s5_ideal": ("s5_rand_observed",
                                 "s5_exec.s5_rand_observed"),
}
...
def main():
    ...
    print("production items: %d (%s + %s)"
          % (len(prod),
             len([k for k in origin if origin[k] == "five_card_rows.v"]),
             len([k for k in origin
                  if origin[k] == "five_card_proximity.v"])))
    ...
    for k in sorted(STAYING):
        ...
        elif where[k] != "five_card_proximity.v":
            fail("a staying declaration moved into a phase file: %s %s in %s"
                 % (k[0], k[1], where[k]))
    in_prox = {k for k in staged if where[k] == "five_card_proximity.v"}
    if in_prox != STAYING:
        fail("the reduced proximity file holds %s, expected %s"
             % (sorted(in_prox), sorted(STAYING)))
    else:
        print("the cut: %d declarations stay in five_card_proximity.v, "
              "%d move" % (len(STAYING), len(prod) - len(STAYING)))

    # 4b. section scaffolding.  Unlike PGL(2,7), this instance has three
    # Sections and two of them move whole.  A declaration's slice runs to
    # the next declaration, so a Section's opening lines ride on the
    # previous declaration's chunk and its End on the last of the section;
    # the split below is therefore checked, not assumed.
    prox_scaf = scaffolding(PROD_PROX)
    rows_scaf = scaffolding(PROD_ROWS)
    expected = {
        "five_card_proximity.v": prox_scaf[:3],
        "five_card_tableau_analysis_bridged.v": rows_scaf + prox_scaf[3:],
    }
    for path in STAGED:
        base = os.path.basename(path)
        got = scaffolding(path)
        want = expected.get(base, [])
        if got != want:
            fail("%s holds section scaffolding %s, expected %s"
                 % (base, got, want))
    if sorted(rows_scaf + prox_scaf) != sorted(
            sum(expected.values(), [])):
        fail("the section scaffolding is not a partition of production's")
    else:
        print("section scaffolding: %d line(s) of production's %d stay in "
              "five_card_proximity.v, %d move with their section into the "
              "AnalysisBridged file, none elsewhere"
              % (len(expected["five_card_proximity.v"]),
                 len(rows_scaf) + len(prox_scaf),
                 len(expected["five_card_tableau_analysis_bridged.v"])))
    ...
    for path in STAGED:
        ...
        # A box line is a line inside a header or banner block, which is
        # to say between an opening and a closing rule line.  A one-line
        # (* ... *) comment written in the source and moved here is not one
        # of them and is not padded to 80 columns.
        depth = 0
        for i, line in enumerate(open(path).read().splitlines(), 1):
            ...
            if r == RULE_LINE:
                depth = 1 - depth
                continue
            if depth and r.startswith("(*") and r.endswith("*)"):
                if r.endswith("**)"):
    ...
    # PGL(2,7) audit ruling 5: a section banner is exactly one content line.
    # Production's rows files have two-line banners, and the generator that
    # re-wraps the first can leave or duplicate the second.
    for path in STAGED:
        base = os.path.basename(path)
        lines = open(path).read().splitlines()
        rules = [k for k, l in enumerate(lines) if l.rstrip() == RULE_LINE]
        for a, b in zip(rules[2::2], rules[3::2]):
            if b - a - 1 != 1:
                fail("%s:%d section banner has %d content lines, not 1"
                     % (base, a + 1, b - a - 1))
    # PGL(2,7) audit ruling 9: no name touches == in an index entry.
    for path in STAGED:
        base = os.path.basename(path)
        for i, line in enumerate(open(path).read().splitlines(), 1):
            if re.match(r"^\(\*   \S+==", line):
                fail("%s:%d index entry name touches ==: %s"
                     % (base, i, line.strip()[:44]))
    print("banners: one content line each; no index name touches ==")
    ...
    prod_prox_last = [l.strip() for l in open(PROD_PROX).read().splitlines()
                      if l.startswith("Local Open Scope")][-1]
    print("innermost scope, production five_card_rows.v: %s" % prod_rows_last)
    print("innermost scope, production five_card_proximity.v: %s"
          % prod_prox_last)
    if prod_prox_last != sorted(set(lasts.values()))[0]:
        print("RE-SCOPED BY DESIGN: the declarations moved out of "
              "five_card_proximity.v are read under the phase files' block. "
    ...
    if prod_rows_last != prod_prox_last:
        print("NOT RE-SCOPED THE OTHER WAY: no declaration of "
              "five_card_rows.v moves into the reduced five_card_proximity.v, "
              "so the re-scoping at this instance runs one way only.")
...
CHECKS = os.path.join(STAGED_DIR, "five_card_tableau_checks.v")


def _unfailed():
    res = {}
    for path, mod in ((PROD_ROWS, "five_card_rows"),
                      (PROD_PROX, "five_card_proximity")):
        for kind, name, body in items(strip_comments(open(path).read())):
...
def check_fails():
    ...
    for name in sorted(UNFAILED):
        ...
        if a != b and name in INTENDED_FAIL_DIFF:
            o, n = INTENDED_FAIL_DIFF[name]
            if a.replace(o, n) == b:
                print("Fail %s: rejection differs BY DESIGN, %r printed as "
                      "%r, the rest byte-identical: the checks file does not "
                      "import s5_exec and production five_card_proximity.v "
                      "does" % (name, o, n))
                continue
        if a != b:
