...
emit(
    ...
    header(
        ...
        ["The Algebraic level of a row holds an algebra and nothing besides, "
         ...
         "instance begin at that algebra, and the uniform analysis and "
         "Kim's two analyses first differ three levels above, at Sampled, "
         "where each names its own model.",
         ...
         "the five card positions in order. Two is the threshold the "
...
""",
    [("The algebra as the first line of a row", [
        doc("The five-card instance at the Algebraic level: the algebra "
            ...
            "mode is built on this value, the committed one: the Executable "
            "file continues it into five_card_committed_executable, and "
            "five_card_committed_executableE identifies that value, with "
            "the three run facts adjoined, with the prefix all seven "
            "published rows continue from.")
...
emit(
    ...
    header(
        ...
        ["The Executable level adjoins run-level data to the algebra: what "
         ...
         "its run is meant to recover one term.",
...
emit(
    ...
    header(
        ...
        [("five_card_committed",
         ...
         ("five_card_F_ite",
          "the ideal function is its conditional spelling, up to "
          "conversion")],
...
""",
    [("The prefix all seven rows share", [
        ...
        doc("The committed program is its own Executable value with the "
            ...
            "equation is what lets a statement made at that named value be "
            "read as a statement about the prefix.")
...
emit(
    ...
    header(
        ...
        ["The Sampled level adjoins a probability model to a run, and what "
         ...
         "Three programs are named here, one per model a published row "
         "continues from, and the three model families they sample are "
         "those of five_card_models.v: all three sit over the one committed "
         "run and are indexed by the unit type. five_card_uniform_family "
         "draws the "
         ...
         "Two of the statements here are about neither a program nor the "
         "manifest's row for it, and no arm of certify takes a payload of "
         "either kind. "
         ...
         "coalition of seats, and it is a bound and not a vanishing. Of the "
         "statements this directory makes it is the one security statement "
         "below AnalysisBridged, and it is of this level for two reasons: "
         "its subject is the law the named Sampled model samples, and the "
         "three statements of certify take an ExactWitness, an "
         "IndistinguishabilityCert and an IdealProximityCert, none of them "
         "a conditional mutual information. The distance "
         "kim_biased_proximity_close of five_card_proximity.v is at no "
         "level and is not counted here.",
        ...
        [("five_card_row_repeated_prefixE",
         ...
         ("five_card_row_repeated_endpoint_lt",
          ...
          "model's cut law is under 2^-40 of the uniform law, in variation "
          "distance"),
         ("kim_centi_small",
...
emit(
    ...
    header(
        ...
        ["The AnalysisBridged level adjoins one security arm to a Sampled "
         ...
         "is a payload or a row is here. Of the statements this directory "
         "makes, the one whose subject is neither a payload nor a row is "
         "Kim's "
         "input-privacy bound, which is in five_card_tableau_sampled.v; the "
         ...
         "the five-card instance. Of the three, the repeated row is the "
         "one whose route the programs here share: the manifest reaches "
         "AnalysisBridged for it by the transfer whose base premise is "
         "kim_centi_cut_mixing, with five_card_static_obs_const for the "
         "reading equality, and kim_centi_cert carries those same two as "
         "fields. For the one-cut row the manifest names two theorems "
         "reaching that level: the corresponding transfer, whose premises "
         "kim_biased_cert carries, and five_card_colour_view_leak_bound, a "
         "conditional mutual information no statement of certify takes and "
         "which is carried at Sampled. For the uniform row the manifest "
         "names five_card_exec_trace_secrecy at that row's own content "
         "trace, where the program's statement is "
         "five_card_exact_view_secrecy, read off the published row by "
         "view_secrecy_of. Of the other "
         ...
         "The map and the five link lemmas of the exact arm are not there "
         "but here, because no proof that stays uses them."],
        ...
        [("five_card_viewS_nth",
         ...
         ("five_card_biased_indistinguishability_implies_proximity",
          "the input-indistinguishability proposition at any constant "
          "implies the proximity proposition at one fiftieth, its "
          "hypothesis unused")]),
    BRIDGED_PRE,
...
emit(
    ...
    header(
        ...
        ["Each entry below is one written term that the kernel rejects, "
         ...
         "differ in two of their five fields.",
    ...
    [("What a model may be sampled over",
      [sub("five_card_row_s5_family",
           ("The two models above are typed",
            "Kim's two models, named at Sampled, are typed"))]),
