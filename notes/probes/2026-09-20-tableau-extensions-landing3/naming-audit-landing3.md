...
identifier cited by a production file. The single rename (N3) is a name new in
...
it (N1); a docstring that says the manifest's row is read off the program when
the manifest declares that row itself (N2); a lemma whose name says it is
about the arms when its statement is about the model family (N3); and two
copies of a sentence about a document outside the development that landing 2
already removed from the sibling file (N4).

Two of the four MUST (N1 in its second half, N4) and eight of the sixteen
SHOULD (N5, N6, N7, N10, N11, N12, N19, N20) are recurrences of defect classes
...
fixed wording now sits. The two MUST that are new to landing 3 are N2, a false
claim about where the manifest's row comes from, and N3, a name that states a
...
measurement sentences are N25 and are inside proofs, which is where the rule
puts proof engineering; the scientific-notation figure is N26. The only
...
Defects: N1, N2, N4, N5, N8, N9, N12, N17, N18, N19, N20, N23, and N7 and N15
...
`4.547e-13`, so N26's figure is right; half of the published `2^-39` is
...
functions is a separate fact. Whether N23's replacement spells the implicit
...
the `E` entries name both sides in every case except N10, which names one half
of a conjunction; the bound entries are N9 (`<=` written as "under") and
correct at `_cert_eps_lt2` (`<`, written as "below"); the number entries are
N14 and correct elsewhere; N13 and N22 are the two remaining wording defects.
Header prose anchored to nothing: true of paragraphs one and four; N1 and N6
...
N21. Rows, certificates, arm equations and recorded failures are presented in
the same shapes and with the same vocabulary as the sibling, with N5, N7, N10,
N11, N13 and N14 the places where the sibling's fixed wording was not carried
across.
...
`word_family`) and its existing wording, which is N30. The names I would
change are N3 (the name states a different object than the statement), N15
(the refutation suffix) and N16 (the home of the one general lemma). The
collision scan is N32 and is clean.

**Remit 5, one word per concept.** Drift found: N12 ("view" as a prose noun,
twice, where the file otherwise says "reading" and reserves "view" for
identifiers), N19 and N20 ("coordinate" in three senses across the landing's
text, one of them clashing with the fixed framework sense in the file it lands
in), N27 ("concludes at" against "published at" for one number), N28 ("prior"
...
**Remit 6, mechanical.** N35, clean on every count.

**Not in remit, not audited.** The thirteen unchanged chain copies under
`staged/`; `landing_fidelity.v` except where N3 and N16 name lines of it;
