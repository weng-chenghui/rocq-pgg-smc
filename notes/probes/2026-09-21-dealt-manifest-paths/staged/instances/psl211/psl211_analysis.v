...
Definition dealt_observed_recovers := OE.oe_run_recovers psl211_dealt_observed.
...
(* The dealer-dealt aliases, one per section that has one. The execution and
   the model keep their dependent indices; the two security aliases are
   pinned at the observers they are stated at, the colour map for the first
   and the framework's static reader for the second. *)
