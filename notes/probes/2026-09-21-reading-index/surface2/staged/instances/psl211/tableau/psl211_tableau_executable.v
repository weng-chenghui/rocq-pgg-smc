...
(* deck, and the value the run is meant to recover is the chirality bit. The  *)
(* three paths the manifest carries for this instance, psl211_alldecks_path,  *)
(* psl211_word_path and psl211_alldecks_obstruction_path, are over this one   *)
(* run. The first two part at the model, and the third parts from the first   *)
(* at the transfer status.                                                    *)
(*                                                                            *)
...
(* carries that record's own reconstruction and termination facts. No program *)
(* of this development continues from it and the manifest carries no path     *)
(* over it, and the only security statement made at it is the refutation      *)
