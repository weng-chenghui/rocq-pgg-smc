...
(* An endpoint reading, read as a static reading: its value type at each
   coalition, and the reading of that coalition's own endpoints through it.
   Every endpoint reading is a function of the dealt cards and the cut alone,
   which is the theorem about a reading that the StaticReading record does not
   state, so the map runs in this direction and not back. *)
Definition static_reading_of_endpoint_reading (r : EndpointReading A)
    : StaticReading :=
  @MkStaticReading (@er_readT A r)
    (fun C x g => @er_of_endpoints A r C (static_coalition_obs C x g)).

(* The identity endpoint reading is a coalition's own static reading. The two
   are the same term, so a proposition stated at the default reading of the
   framework is the proposition this file states at that coalition reading. *)
Lemma static_reading_of_coalition_endpoint_readingE :
  static_reading_of_endpoint_reading (coalition_endpoint_reading A)
  = static_coalition_reading.
Proof. exact: erefl. Qed.

End static_reading.

Arguments StaticReading {A} E.
Arguments static_reading_of_endpoint_reading {A} E r.
...
(* At the static reading an endpoint reading gives, the proposition at a
   reading is the framework's proposition at that endpoint reading, at every
   certificate and every number.  The lemma above is this one at the identity
   endpoint reading. *)
Lemma reading_indistinguishability_endpointE (r : EndpointReading A)
    (cert : IndistinguishabilityCert sa r) (c : R) :
  ReadingIndistinguishabilityPropAt (static_reading_of_endpoint_reading E r) c
  = IndistinguishabilityPropAt cert c.
Proof. exact: erefl. Qed.
...
(* An exact-independence witness is exact independence at a coalition's static
   ...
   proposition. *)
Definition exact_independence_of_witness
    (w : ExactWitness sa (coalition_endpoint_reading A))
  : ReadingExactIndependence (static_coalition_reading E) (ew_secret w) :=
  @ew_indep _ _ _ _ _ w.

(* The same at every endpoint reading: a witness indexed by r is exact
   independence at the static reading r gives, with no proof. *)
Definition exact_independence_of_witness_at (r : EndpointReading A)
    (w : ExactWitness sa r)
  : ReadingExactIndependence (static_reading_of_endpoint_reading E r)
      (ew_secret w) :=
  @ew_indep _ _ _ _ _ w.
