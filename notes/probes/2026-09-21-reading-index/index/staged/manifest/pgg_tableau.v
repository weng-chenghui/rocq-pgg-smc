...
(* The witness at a named reading, in the shape certify_exact wants. A
   statement writes the reading once and the witness is checked against it
   where it is written. *)
Definition exact_of_reading (x : StackAt Sampled)
    (r : EndpointReading (projT1 x))
    (w : forall (R : realType) (idx : amf_index (sp_f x) R),
           ExactWitness (amf_sample (sp_f x) R idx) r) : ExactPayload x :=
  existT _ r w.
...
(* The certificate at a named reading, in the shape
   certify_indistinguishability wants. *)
...
(* The proximity certificate at a named reading, in the shape
   certify_idealproximity wants. *)
Definition idealproximity_of_reading (x : StackAt Sampled)
    (r : EndpointReading (projT1 x))
    (c : forall (R : realType) (idx : amf_index (sp_f x) R),
           IdealProximityCert (amf_sample (sp_f x) R idx) r)
    : IdealProximityPayload x :=
  existT _ r c.
...
Definition certify_indistinguishability (x : StackAt Sampled)
  ...
  @MkTableau AnalysisBridged (StackProp AnalysisBridged)
    (existT _ (projT1 x) (existT _ (projT1 (projT2 x))
       (existT _ (sp_Ht x) (existT _ (sp_He x) (existT _ (sp_Hr x)
          (existT _ (sp_f x)
             (fun R idx => InputIndistinguishability (projT2 p R idx))))))))
