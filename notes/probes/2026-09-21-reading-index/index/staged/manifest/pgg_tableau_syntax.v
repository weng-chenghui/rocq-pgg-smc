...
Definition mk_indistinguishability (x : StackAt Sampled)
    ...
    (Hk : forall (R : realType) (idx : amf_index (sp_f x) R)
            ...
            forall y y' : ex_inputT (projT1 (projT2 x)),
              fdistmap (fun g => @er_of_endpoints (projT1 x) r C
                          (static_coalition_obs C y g)) (u R idx)
              = fdistmap (fun g => @er_of_endpoints (projT1 x) r C
                            (static_coalition_obs C y' g)) (u R idx))
    : IndistinguishabilityPayload x :=
  existT _ r (fun R idx =>
    @MkIndistinguishabilityCert R (projT1 x) (projT1 (projT2 x))
      (amf_sample (sp_f x) R idx) r
...
Definition ExactLeakAt (k : nat) (x : StackAt Sampled) (p : ExactPayload x)
  ...
  forall (R : realType) (idx : amf_index (sp_f x) R),
    exists C : {set 'I_(pi_T' (mp_PI (instance_profile (projT1 x)))).+1},
      #|C| = k
      /\ 0 < `I( ew_secret (projT2 p R idx) ;
                 (fun u => @er_of_endpoints (projT1 x) (projT1 p) C
...
(* The bare spellings, at the coalition's own endpoint reading. A program
   naming no reading is a program at that one, so the text of a program
   written before the reading index reads as it read then. *)
Notation "s 'certify' 'ExactIndependence' w" :=
  (s ;;; certify_exact
     of (@exact_of_reading (tableau_at s) _ w))
  (at level 90, left associativity, w at level 0).

(* The reading is written after the property's name, and the clause is a type
   ascription on the evidence: a witness whose index is another reading is
...
Notation "s 'certify' 'ExactIndependence' w 'leaks' 'at' k 'by' H" :=
  (s ;;; certify_exact
     of (exact_leaks (tableau_at s)
           (@exact_of_reading (tableau_at s) _ w)
           k H))
  (at level 90, left associativity, w at level 0, k at level 0, H at level 0,
   only parsing).

Notation "s 'certify' 'InputIndistinguishability' c" :=
  (s ;;; certify_indistinguishability
     of (@indistinguishability_of_reading (tableau_at s) _ c))
...
Notation "s 'certify' 'IdealProximity' c" :=
  (s ;;; certify_idealproximity
     of (@idealproximity_of_reading (tableau_at s) _ c))
  (at level 90, left associativity, c at level 0).
...
Notation "s 'certify' 'InputIndistinguishability' 'at' R idx b 'tied' 'by' Hd 'ideal' u 'mixing' 'by' Hc 'invariant' 'by' Hk" :=
  (s ;;; certify_indistinguishability
     of (mk_indistinguishability (tableau_at s) (coalition_endpoint_reading _)
