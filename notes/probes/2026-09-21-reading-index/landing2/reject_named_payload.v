(* The obstruction terminal's recorded rejection, compiled unguarded so that
   the parser's own message is read and not assumed. The term is the all-decks
   obstruction program with the named payload written in the kind's position.
   The file is expected to fail; it is kept as the record of the message. *)
...
Definition rejected : PublishedObstruction :=
