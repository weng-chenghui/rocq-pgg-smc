...
Record CoalitionReading (A : PGGAlgebraic) := MkCoalitionReading {
  ...
  cr_read : forall C : {set 'I_(pi_T' (mp_PI (instance_profile A))).+1},
    {ffun 'I_(pi_T' (mp_PI (instance_profile A))).+1 -> 'I_(pgg_N' (mp_M (instance_profile A))).+1} -> cr_readT C }.
...
(* The exact-independence proposition: below the threshold, the coalition's
   ...
   beside it. *)
(* The exact-independence proposition of a witness: below the threshold, the
   coalition's executed view THROUGH THE WITNESS'S OWN READING is independent
   of the secret, its mutual information with the secret is zero,
   conditioning on it leaves the secret's entropy unchanged, and every
   deterministic function of it is still independent.  The reading is not
   written here because the witness carries it, so a program's text cannot
   name a reading its evidence is not about. *)
...
(* The ideal-proximity proposition of a certificate, at the reading the
   certificate is indexed by: below the threshold, the joint law of the
   coalition's reading of the executed run with the secret is within c of the
   product of the ideal model's two marginals, its own reading and its own
   secret.  The number bounds a sum of absolute differences, twice the total
...
(* Concluding a program at a chosen number leaves its security property where
   ...
   own bound. *)
(* A program whose certify statement names no reading carries the
   coalition's own endpoint reading, at every real field and every index of
   its family.  With the two siblings below and publish_propertyE it settles
   the observer coordinate of a finished program by one line of the
   program's text, as certify_exact_propertyE settles the property. *)
...
(* A program whose certify statement names a reading carries that reading,
   at every real field and every index. *)
Lemma certify_reading_exact_readingE (x : StackAt Sampled)
    (q : StackProp Sampled x) (p : ExactPayloadOfReading x) (R : realType)
    (idx : amf_index (ab_f (tableau_at (@certify_reading_exact x q p))) R) :
  ab_reading (tableau_at (@certify_reading_exact x q p)) R idx = projT1 p.
Proof. by []. Qed.
