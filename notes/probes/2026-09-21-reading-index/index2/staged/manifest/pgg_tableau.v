...
(*                                                                            *)
(* Definitions:                                                               *)
(*   EndpointReading        == what a coalition is granted to read off its    *)
(*                             own endpoints                                  *)
(*   coalition_endpoint_reading                                               *)
(*                          == the identity reading, a coalition's own        *)
(*                             endpoints                                      *)
...
(*   ExactPropAtReading     == the exact-independence proposition at a        *)
(*                             reading                                        *)
(*   IndistinguishabilityPropAtReading                                        *)
(*                          == the input-indistinguishability proposition at  *)
(*                             a reading                                      *)
(*   IdealProximityPropAtReading                                              *)
(*                          == the ideal-proximity proposition at a reading   *)
...
Record EndpointReading (A : PGGAlgebraic) := MkEndpointReading {
  ...
  er_of_endpoints : forall C : {set 'I_(pi_T' (mp_PI (instance_profile A))).+1},
    {ffun 'I_(pi_T' (mp_PI (instance_profile A))).+1 -> 'I_(pgg_N' (mp_M (instance_profile A))).+1} -> er_readT C }.

(* A coalition's own endpoints, read as a reading: the identity at every
   coalition. It is the reading of every proposition of this file before the
   index was added, so a program naming no reading is a program at this one
   and states what it stated before. *)
Definition coalition_endpoint_reading (A : PGGAlgebraic) : EndpointReading A :=
  @MkEndpointReading A
    (fun _ => [the finType of {ffun 'I_(pi_T' (mp_PI (instance_profile A))).+1 -> 'I_(pgg_N' (mp_M (instance_profile A))).+1}])
    (fun _ v => v).
...
Record ExactWitness (R : realType) (A : PGGAlgebraic)
  ...
  MkExactWitness {
    ...
    ew_indep   : forall C : {set 'I_(pi_T' (mp_PI (instance_profile A))).+1},
      ...
      sa_sampleP sa |= (fun u => @er_of_endpoints A r C
...
(* The reading the evidence is stated at, with its witness or its certificate
   forgotten. The constructor carries it, so the observer coordinate of a
   finished program is read off one line of the program's text, as its
...
(* the reading of each constructor is implicit, so the three patterns name the
   witness or the certificate alone *)
...
(* The exact-independence proposition: below the threshold, the coalition's
   ...
   beside it. *)
Definition ExactPropAtReading (R : realType) (A : PGGAlgebraic)
    (E : ExecutionParams A) (sa : SampleAdapter R (instance_exec E))
    (r : EndpointReading A) (secretT : finType)
  ...
  forall C : {set 'I_(pi_T' (mp_PI (instance_profile A))).+1},
    ...
    [/\ sa_sampleP sa |= (fun u => @er_of_endpoints A r C
                         ...
                         _|_ secret,
        `I( secret ;
            (fun u => @er_of_endpoints A r C
               (@sa_coalition_view R (instance_profile A) (instance_exec E)
                  sa 0 C u)) ) = 0,
        `H( secret |
            (fun u => @er_of_endpoints A r C
               (@sa_coalition_view R (instance_profile A) (instance_exec E)
                  sa 0 C u)) ) = `H `p_ secret
      & forall (W : finType) (h : @er_readT A r C -> W),
          sa_sampleP sa
          |= (h `o (fun u => @er_of_endpoints A r C
                       (@sa_coalition_view R (instance_profile A)
                          (instance_exec E) sa 0 C u)))
             _|_ secret].
Arguments ExactPropAtReading {R A E} sa r {secretT} secret.

(* The exact-independence proposition of a witness: the proposition above at
   the reading the witness is indexed by and at the witness's own secret. The
   reading is not written here because the witness carries it, so a program's
   text cannot name a reading its evidence is not about. *)
Definition ExactProp (R : realType) (A : PGGAlgebraic)
    (E : ExecutionParams A) (sa : SampleAdapter R (instance_exec E))
    (r : EndpointReading A) (w : ExactWitness sa r) : Prop :=
  ExactPropAtReading sa r (ew_secret w).
...
Definition IndistinguishabilityPropAtReading (R : realType) (A : PGGAlgebraic)
    (E : ExecutionParams A) (sa : SampleAdapter R (instance_exec E))
    (r : EndpointReading A) (c : R) : Prop :=
  forall (C : {set 'I_(pi_T' (mp_PI (instance_profile A))).+1})
         (x x' : ex_inputT E),
    (#|C| < profile_k (instance_profile A))%N ->
    var_dist (fdistmap (fun g => @er_of_endpoints A r C
                          (static_coalition_obs C x g)) (sa_cut_dist sa))
             (fdistmap (fun g => @er_of_endpoints A r C
                          (static_coalition_obs C x' g)) (sa_cut_dist sa))
    <= c.
Arguments IndistinguishabilityPropAtReading {R A E} sa r c.

(* The input-indistinguishability proposition of a certificate: the
   proposition above at the reading the certificate is indexed by. *)
Definition IndistinguishabilityPropAt (R : realType) (A : PGGAlgebraic)
    (E : ExecutionParams A) (sa : SampleAdapter R (instance_exec E))
    (r : EndpointReading A) (cert : IndistinguishabilityCert sa r) (c : R)
    : Prop :=
...
Definition IdealProximityPropAtReading (R : realType) (A : PGGAlgebraic)
    (E : ExecutionParams A) (sa : SampleAdapter R (instance_exec E))
    (r : EndpointReading A) (ideal : SampleAdapter R (instance_exec E))
    (secretT : finType) (secret : {RV (sa_sampleP sa) -> secretT})
    (isecret : {RV (sa_sampleP ideal) -> secretT}) (c : R) : Prop :=
  forall C : {set 'I_(pi_T' (mp_PI (instance_profile A))).+1},
    (#|C| < profile_k (instance_profile A))%N ->
    var_dist
      (fdistmap (fun u => (@er_of_endpoints A r C
                             (@sa_coalition_view R (instance_profile A)
                                (instance_exec E) sa 0 C u), secret u))
         (sa_sampleP sa))
      ((fdistmap (fun u => @er_of_endpoints A r C
                    (@sa_coalition_view R (instance_profile A)
                       (instance_exec E) ideal 0 C u)) (sa_sampleP ideal))
       `x (fdistmap isecret (sa_sampleP ideal)))
    <= c.
Arguments IdealProximityPropAtReading {R A E} sa r ideal {secretT} secret
  isecret c.

(* The ideal-proximity proposition of a certificate: the proposition above at
   the reading the certificate is indexed by, at its ideal model, its own
   secret and the ideal's secret. *)
Definition IdealProximityPropAt (R : realType) (A : PGGAlgebraic)
    (E : ExecutionParams A) (sa : SampleAdapter R (instance_exec E))
    (r : EndpointReading A) (cert : IdealProximityCert sa r) (c : R) : Prop :=
  IdealProximityPropAtReading sa r (ipc_ideal cert) (ipc_secret cert)
    (ew_secret (ipc_witness cert)) c.
...
(* The payload of the statement that names a reading: the reading, and one
   witness per real field and index at it.  It is a separate type from the
   one above, and the statement below is a separate statement, because a
   program that names no reading must carry a payload that mentions no
   reading: the payload of such a program is a term of the instance and the
   equations between two spellings of one program are decided by conversion,
   which a pair carrying the program's own stack coordinate in its type
   makes expensive. *)
...
(* The witness at a named reading.  A statement writes the reading once and
   the witness is checked against it where it is written. *)
...
(* The payload of the statement that names a reading, on the same footing as
   the exact one. *)
...
apply: (var_dist_fdistmap_transfer R _ _ (sa_cut_dist sa) (ic_ideal cert)
  (fun g => @er_of_endpoints A r C (static_coalition_obs C x g))
  (fun g => @er_of_endpoints A r C (static_coalition_obs C x' g))
...
have Hconst : fdistmap (fun g => @er_of_endpoints A r C
                          (static_coalition_obs C x g)) (ic_ideal cert)
            = fdistmap (fun g => @er_of_endpoints A r C
                          (static_coalition_obs C x' g)) (ic_ideal cert)
...
Proof.
move=> C HC.
rewrite /IdealProximityPropAtReading (Hview C) (Hideal C).
...
(* The same statement at a reading the program names.  The two statements
   differ in their payload alone; the data they build and the proposition
   they prove are the same, and the reading is read back off either by
   evidence_reading. *)
Definition certify_exact_of_reading (x : StackAt Sampled)
...
Arguments certify_exact_of_reading x q p : assert.
...
(* The same statement at a reading the program names. *)
Definition certify_indistinguishability_of_reading (x : StackAt Sampled)
    (q : StackProp Sampled x)
...
Arguments certify_indistinguishability_of_reading x q p : assert.
...
(* The same statement at a reading the program names. *)
Definition certify_idealproximity_of_reading (x : StackAt Sampled)
...
Arguments certify_idealproximity_of_reading x q p : assert.
...
Definition InputDistinguishabilityPropAt (R : realType) (A : PGGAlgebraic)
  ...
  exists (C : {set 'I_(pi_T' (mp_PI (instance_profile A))).+1})
         (x x' : ex_inputT E),
    (#|C| < profile_k (instance_profile A))%N /\
    c <= var_dist (fdistmap (fun g => @er_of_endpoints A r C
                               (static_coalition_obs C x g)) (sa_cut_dist sa))
                  (fdistmap (fun g => @er_of_endpoints A r C
...
Variant ObstructionKind (R : realType) (A : PGGAlgebraic)
    (E : ExecutionParams A) (sa : SampleAdapter R (instance_exec E)) : Type :=
  | InputDistinguishabilityObstruction of EndpointReading A & R.
