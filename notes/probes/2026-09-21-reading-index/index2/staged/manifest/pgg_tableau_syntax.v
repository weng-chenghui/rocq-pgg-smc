...
(* The reading is written after the property's name.  The clause is a type
   ascription on the evidence: the payload's type is the certificate at r, so
   a certificate whose index is another reading is rejected where it is
   written and a program's text cannot name a reading its evidence is not
   about.  A statement that omits the clause is a statement at the
   coalition's own endpoint reading, and expands to the rule above, whose
   payload mentions no reading at all. *)
Notation "s 'certify' 'ExactIndependence' 'of' r 'by' w" :=
  (s ;;; certify_exact_of_reading
...
Notation "s 'certify' 'InputIndistinguishability' 'of' r 'by' c" :=
  (s ;;; certify_indistinguishability_of_reading
...
Notation "s 'certify' 'IdealProximity' 'of' r 'by' c" :=
  (s ;;; certify_idealproximity_of_reading
