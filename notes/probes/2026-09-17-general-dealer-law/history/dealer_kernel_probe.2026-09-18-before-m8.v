...
(** dealer_view_indep — when every secret of positive mass sends the pair of
    deck and shuffle to one and the same law on observations, the observation is
...
(** dealer_view_indep_of_deck — the same conclusion from a per-deck premise:
    every deck of positive mass is valid, and every valid deck sends the shuffle
    law alone to one law on observations, uniformly in the secret.  This is
    the form a transitivity argument produces, where the shuffle already erases
...
Definition dealer_shuffle_view_indep_with_common_law :
  ...
  @dealer_view_indep R secretT deckT shuffleT viewT secretP delta nu
    view (fdist1 x) Hmix.
