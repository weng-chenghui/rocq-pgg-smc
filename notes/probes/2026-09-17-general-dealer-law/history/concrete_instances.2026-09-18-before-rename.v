...
(** pgl27_view_indep_realE — over the standard real line, a coalition of at
    most three card positions has a view of the cut deck independent of the
...
Lemma pgl27_view_indep_realE (C : {set 'I_8}) :
  (#|C| <= 3)%N ->
  pgl27P Rc |= pgl27_view Rc C _|_ pgl27_secret Rc.
Proof. exact: (@pgl_indep_via_generic Rc C). Qed.

(** pgl27_view_indep_alldecks_realE — the same over the standard real line for
...
Lemma pgl27_view_indep_alldecks_realE (C : {set 'I_8}) :
  (#|C| <= 3)%N ->
...
Proof. exact: (@pgl_all_indep_via_generic Rc C). Qed.

(** psl211_alldecks_view_indep_realE — over the standard real line, a
    ...
    the dealer model reaches the conclusion only through the average over deal
    descriptions, which is the difference from the two PGL rows above. *)
Lemma psl211_alldecks_view_indep_realE (C : {set 'I_12}) :
  (#|C| <= 5)%N ->
  psl211_alldecksP Rc |= (fun u => psl211_alldecks_view C u.1 u.2)
                     _|_ psl211_alldecks_secret Rc.
Proof. exact: (@psl_indep_via_generic Rc C). Qed.

(** psl211_dealer_lawE_real — the dealer model's pointwise factorization at
    ...
    object. *)
Lemma psl211_dealer_lawE_real (b : bool) (d : psl211_deal)
    (g : pgg_gT psl211_M) :
  psl_genericP Rc (b, (d, g)) =
  (fdist_uniform card_bool : Rc.-fdist bool) b *
    (psl_delta Rc b d * psl_nu Rc g).
Proof. by rewrite /psl_genericP dealer_shufflePE. Qed.

Print Assumptions pgl27_view_indep_realE.
Print Assumptions pgl27_view_indep_alldecks_realE.
Print Assumptions psl211_alldecks_view_indep_realE.
Print Assumptions psl211_dealer_lawE_real.
