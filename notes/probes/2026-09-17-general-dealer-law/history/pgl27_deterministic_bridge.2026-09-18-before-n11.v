...
(** pgl27_alldecks_dealer_view_law — at every deck without repeated cards,
    ...
    dealer model.  Validity is the last premise so that dropping it leaves an
    arrow rather than shifting an argument. *)
...
(** pgl27_view_indep_alldecks_via_dealer — under the all-decks dealer, the
    ...
    secret.  It is dealer_shuffle_view_indep_of_deck with validity taken to be a
    deck without repeated cards: every deck of a class has that property, and
    every deck with it has the same view law, so the average over decks is not
    needed and the conclusion holds deck by deck. *)
