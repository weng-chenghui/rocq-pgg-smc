...
(** dealer_shuffleP — the joint law of a secret, the deck a dealer lays for
    that secret, and the cut applied to the laid deck: the secret is drawn
    from secretP, the deck from delta at the drawn secret, and the cut from nu
    independently of both.  Every privacy statement of the development is an
    average over this space, and the order of the three draws is the modelling
    decision it fixes: the deck may depend on the secret, the cut may not. *)
Definition dealer_shuffleP : R.-fdist (secretT * (deckT * cutT)) :=
  secretP `X (fun s => (delta s) `x nu).

(** dealer_secret — the secret coordinate of a sample, read as a random
    variable.  It is the quantity a coalition must not learn, and the second
    argument of every independence statement below. *)
Definition dealer_secret : {RV dealer_shuffleP -> secretT} :=
  fun u => u.1.

(** dealer_view view — what a coalition observes at a sample, for an
    observation function view of the secret, the deck and the cut.  Letting
...
(** dealer_shufflePE — the pointwise factorization of the joint law into the
    secret prior, the dealer kernel at that secret, and the cut law.  It is
...
(** dealer_view_indep — when every secret of positive mass sends the pair of
    deck and cut to one and the same law on observations, the observation is
    independent of the secret.  The statement is exact and
...
(** dealer_view_indep_of_deck — the same conclusion from a per-deck premise:
    every deck of positive mass is valid, and every valid deck sends the cut
    law alone to one law on observations, uniformly in the secret.  This is
    the form a transitivity argument produces, where the cut already erases
    the secret at each fixed deck.  It is strictly stronger than what
    dealer_view_indep needs, so an instance whose symmetry appears only after
    averaging over decks cannot reach its conclusion this way. *)
...
Proof.
move=> Hvalid Hdeck.
apply: (@dealer_view_indep view mu) => s Hs.
...
(* Expected failure: a dealer law with no cut.  The ascribed type is a law on
   secretT * (deckT * cutT), while secretP `X delta is a law on
   secretT * deckT, so the two types do not unify. *)
Fail Definition dealer_shuffleP_missing_cut :
    R.-fdist (secretT * (deckT * cutT)) :=
  secretP `X delta.

(** dealer_view_indep_with_common_law — the positive control for the mutation
    below: with the common mixed law supplied, the same spelling of
    dealer_view_indep is the independence statement. *)
Definition dealer_view_indep_with_common_law :
  ...
  @dealer_view_indep R secretT deckT cutT viewT secretP delta nu
    view (fdist1 x) Hmix.
