...
(** psl211_perdeck_no_common_law — no law on readings is the law of the
    ...
    it has no solution: the per-deck route to privacy is closed for this
    instance, and only the average over deals that dealer_shuffle_view_indep
    asks for remains. *)
...
(** psl211_dealer_view_indep_of_deck_unsat — at PSL(2,11) under its own dealer
    ...
    The per-deck route to privacy is closed for this instance, and the average
    over deal descriptions that dealer_shuffle_view_indep asks for is the only
    route that remains. *)
...
(** psl211_fixed_deal_delta — the degenerate dealer that lays one and the same
    ...
    can be asked whether it claims privacy for it. *)
...
(** psl211_fixed_deal_view_dep — under that dealer law the reading of three
    ...
    PSL(2,11) privacy is a property of the uniform average over deal
    descriptions.  What is NOT shown here is that a hidden uniform deal
    leaks: the deal is public in this refutation, being a point mass. *)
...
have Hemp : psl211_perdeck_fiber true = set0.
  ...
  exact: etrans (psl211_perdeck_fiberE true) Ht.
...
have /card_gt0P[g0 Hg0] : (0 < #|psl211_perdeck_fiber false|)%N.
  by rewrite (etrans (psl211_perdeck_fiberE false) Hf).
