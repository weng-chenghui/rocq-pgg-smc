...
(** fdistmap_prod_sections — two observations of an independent pair agree in
...
(** fdistmap_prod_sections_with_sections — the positive control for the
    mutation below: with the section-wise premise supplied, the same spelling
    is the equality of the two laws on the pair. *)
Definition fdistmap_prod_sections_with_sections :
  fdistmap (fun dg => f dg.1 dg.2) (PD `x PG) =
  fdistmap (fun dg => h dg.1 dg.2) (PD `x PG) :=
  @fdistmap_prod_sections R D G V PD PG f h Hs.
...
Fail Definition fdistmap_prod_sections_without_sections :
  fdistmap (fun dg => f dg.1 dg.2) (PD `x PG) =
  fdistmap (fun dg => h dg.1 dg.2) (PD `x PG) :=
  @fdistmap_prod_sections R D G V PD PG f h.
...
(** psl_deal_pos — the type of deal descriptions is inhabited, so it carries a
    uniform law. *)
...
(** psl_delta — the all-decks dealer: whatever the chirality, the deal
    description is drawn uniformly and independently of it.  This is the
...
(** psl_nu — the cut law: a uniform element of the PSL(2,11) shuffle group. *)
Definition psl_nu : R.-fdist cutT := `U psl211_G_pos.

(** psl_genericP — the instance's data placed in the dealer model's sample
...
(** psl_assoc — the reassociation of the instance's sample space, which pairs
    a chirality with a deal before pairing with the cut, into the dealer
    model's, which pairs the chirality with the pair.  It moves no mass. *)
Definition psl_assoc
    (u : psl211_inputT * cutT) : bool * (dealT * cutT) :=
  (u.1.1, (u.1.2, u.2)).

(** psl_law_E — the reassociation carries the instance's all-decks law to the
    dealer model's law at psl_delta and psl_nu.  Both are uniform on the same
    finite set written in two associations, so the identification costs
    nothing probabilistic and fixes which of the dealer model's three draws
    each of the instance's coordinates is. *)
Lemma psl_law_E :
...
rewrite dealer_shufflePE /psl_delta /psl_nu.
...
(** psl_generic_view C — the reading of a coalition C of seats, as a function
...
(** psl_view_square — the dealer model's reading, composed with the
    reassociation, is the instance's reading. *)
Lemma psl_view_square (C : {set seatT}) :
  @dealer_view R bool dealT cutT viewT
    (fdist_uniform card_bool) psl_delta psl_nu
    (psl_generic_view C) \o psl_assoc =
  (fun u => psl211_alldecks_view C u.1 u.2).
Proof. by apply: boolp.funext => -[[b d] g]. Qed.

(** psl_secret_square — the dealer model's secret, composed with the
    reassociation, is the instance's chirality. *)
Lemma psl_secret_square :
  @dealer_secret R bool dealT cutT
    (fdist_uniform card_bool) psl_delta psl_nu \o psl_assoc =
  psl211_alldecks_secret R.
Proof. by []. Qed.

(** psl_bad_assoc — the reassociation with the chirality negated, used only to
    show that the identification of the secret is not automatic. *)
...
(** psl_mixed_law C b — the law of the coalition's reading at chirality b,
    averaged over the deal and the cut.  It is the quantity the dealer model's
    privacy premise asks to be the same for both chiralities. *)
...
(** psl_section_law — at one cut, the two chiralities send the uniform law on
    deal descriptions to the same law on what a coalition of at most five
...
(** psl_mixed_lawE — the two chiralities have the same averaged reading law.
    The per-cut equality is lifted to the pair of deal and cut, which is the
    dealer model's privacy premise at this instance. *)
...
apply: (@fdistmap_prod_sections R dealT cutT viewT
  ...
  (fun d g => psl_generic_view C true d g)) => g _.
symmetry; exact: (@psl_section_law C g HC).
Qed.

(** psl_generic_indep — in the dealer model's sample space, the reading of a
    coalition of at most five seats is independent of the chirality.  This is
    dealer_view_indep at the common averaged law, and it is exact: no
...
Lemma psl_generic_indep (C : {set seatT}) :
  ...
  psl_genericP |=
    @dealer_view R bool dealT cutT viewT
      (fdist_uniform card_bool) psl_delta psl_nu
      (psl_generic_view C)
    _|_ @dealer_secret R bool dealT cutT
      (fdist_uniform card_bool) psl_delta psl_nu.
Proof.
move=> HC.
...
(** psl_indep_via_generic — the instance's own privacy statement, obtained by
    transporting the dealer model's independence back across the
    reassociation.  Nothing of the counting argument is redone here: the deal
    count of psl211_alldecks entered at psl_section_law and this step only
    changes the coordinates the same law is written in. *)
...
(* the two square equations are rewritten into the goal, not into Hgen: the
...
Print Assumptions fdistmap_prod_sections.
