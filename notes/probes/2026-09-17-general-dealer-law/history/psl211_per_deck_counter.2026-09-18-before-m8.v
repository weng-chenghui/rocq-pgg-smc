...
(* Only raw nat data reduces here.  `inord` is `insubd ord0`, whose `insub`
   ...
   finfun or finset over 'I_12 go through `inord` or `ord_enum`, the counted
   term below names none of them: it is a `count` over the raw permutation
   tables, and the three lemmas psl211_perdeck_seqE, psl211_perdeck_raw_viewE
   and psl211_perdeck_testE carry it back to the deck, the reading and the
   shuffle group symbolically. *)
...
(** psl211_perdeck_fiber_card_neq — at one deck description the two
    ...
    deal, so a general dealer law cannot be obtained by fixing a representative
    deck. *)
