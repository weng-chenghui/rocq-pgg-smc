...
(* number. The input-indistinguishability certificate spends it once for each *)
(* of the two dealt secrets it compares, so its cert_eps is that number added *)
(* to itself, 2^-39. The proximity certificate compares one law with one law  *)
(* and carries the number itself, 2^-40, and the proximity row concludes at   *)
(* 2^-39, the constant the other row over the same model publishes, so the    *)
(* terminal's obligation is met strictly. Each of these numbers bounds a sum  *)
(* of absolute differences, twice a total variation distance, so a            *)
(* distinguisher's advantage against a row concluded at 2^-39 is at most      *)
(* 2^-40. Four is the threshold the derived profile declares, so every        *)
(* statement here that quantifies over a coalition quantifies over at most    *)
(* three of the eight seats, each seat reading the card at its own position,  *)
...
(* write the same claims again: the word row concluded at 2^-39 in three      *)
(* spellings, through the surface, through the raw bind and from the named    *)
(* Sampled value, and the proximity row, which publishes the manifest's word  *)
(* row a second time, an AnalysisPathRow holding descriptive metadata and no  *)
(* Prop. The manifest carries no fourth row over this instance and publishes  *)
(* none of the three by a route this development's programs do not take.      *)
(*                                                                            *)
...
(* Executable value, the parameter equation and the two identifications of a  *)
(* coalition's reading; the observed file the two prefixes, the inline fork's *)
(* parameter equation and the ideal functionality; the sampled file the three *)
(* named models; this file the payloads, the rows, the row and arm equations, *)
(* the bridges, the restated theorems and the two arm statements; and the     *)
(* checks file the recorded rejections and the comparison of the two word     *)
(* rows' arms. No file of the six uses Require Export.                        *)
(*                                                                            *)
(* This file requires instances/pgl27/pgl27_proximity.v, which holds the      *)
(* distance mathematics the proximity certificate is built from: the dealt    *)
(* secret on the word sample space, the distance between the two models'      *)
(* joint laws, and the two arithmetic facts about 2^-40 the conclude          *)
...
(*   pgl27_row_word_proximity== the word row as a program at the proximity    *)
...
(*   pgl27_row_exact_sampledE== the exact row continues the named exact model *)
...
(*   pgl27_exact_view_secrecy== the exact arm's four conjuncts at this        *)
(*                              instance                                      *)
