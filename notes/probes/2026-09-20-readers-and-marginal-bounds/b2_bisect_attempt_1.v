(* PROBE bisect file 2: the remaining commands of r_framework.v, each
...
Timeout 90 Definition c1_postprocessing
    (r r' : StaticReader E)
...
Timeout 90 Definition c2_exact_of_witness (w : ExactWitness sa)
  : ReaderExactPropAt (coalition_reading_reader E) (ew_secret w) :=
  @ew_indep _ _ _ _ w.

(* c3: the link lemma carries it to the executed view. *)
Timeout 90 Definition c3_exact_executed (secretT : finType)
  ...
  forall C : {set seats}, (#|C| < profile_k (instance_profile A))%N ->
    ...
    |= (@sa_coalition_view R (instance_profile A) (instance_exec E) sa 0 C)
       _|_ secret
  := ltac:(move=> H C HC; rewrite (Hview C); exact: H).
...
Timeout 90 Definition c4_seat_at_two (i : seats)
    (ideal : R.-fdist 'I_(pgg_N' (mp_M (instance_profile A))).+1) :
  SeatMarginalPropAt i ideal 2%:R
  := ltac:(exact: var_dist_le2).

(* c5: the same at the cut form. *)
Timeout 90 Definition c5_cut_at_two (T : finType)
...
Timeout 90 Fail Check (fun (i : seats)
                (ideal : R.-fdist 'I_(pgg_N' (mp_M (instance_profile A))).+1)
...
Timeout 90 Fail Definition c8_no_factorisation
