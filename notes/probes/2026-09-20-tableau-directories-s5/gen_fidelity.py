#!/usr/bin/env python3
"""Generate fidelity.v (against the staged tableau/ files) and baseline.v
(against production's s5_rows.v) from one shared body.

Both files ascribe every name instances/s5/s5_rows.v declares at the
statement it has in production, so a lost or altered declaration is a compile
error, and both print the assumptions of every published row and every lemma,
so the two outputs can be diffed.  Only the Require line differs.
"""
...
# Every name s5_rows.v declares, ascribed at its production statement.
ASCRIPTIONS = r"""
...
write("fidelity.v",
      "From pgg_smc Require Import s5_tableau_algebraic s5_tableau_executable.\n"
      ...
      "From pgg_smc Require Import s5_tableau_checks.\n",
      NEW)
