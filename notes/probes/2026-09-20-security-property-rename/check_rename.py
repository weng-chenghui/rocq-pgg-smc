#!/usr/bin/env python3
"""Checks that the rename of the security-property and security-evidence
identifiers changed identifiers only.
...
# A file the rename moves: path at BASE -> path in the working tree.
PATH_MAP = {"manifest/pgg_tableau_arm_relations.v":
...
def main():
    ...
    for f in files:
        ...
        path = os.path.join(REPO, PATH_MAP.get(f, f))
        if not os.path.exists(path):
            print(f"MISSING {f}")
            ok = False
            continue
        new = open(path, encoding="utf-8").read()
        if old == new and f not in PATH_MAP:
