...
(* to one of them and claims nothing about the other, and security_claim_of   *)
(* names which of the two a finished row committed to.                        *)
(*                                                                            *)
...
(* outside: it leaves the data and the arms untouched and moves the real the  *)
(* spectral arm's proposition mentions to any upper bound of it.              *)
(*                                                                            *)
(* Definitions:                                                               *)
...
(*   stack_claim            == the claim a row's data carries, at one field   *)
(*                             and one index                                  *)
...
(*   security_claim_of      == which of the two claims a published row        *)
(*                             carries                                        *)
(*                                                                            *)
(* Key results:                                                               *)
...
(*   port_reprice           == a port's proposition at an upper bound of its  *)
(*                             own                                            *)
(******************************************************************************)
...
(* The claim a port carries, with the witness or the certificate forgotten.
   The port determines the claim by its constructor, so the answer is decided
   by conversion at every program that writes one. *)
...
(* The obligation of conclude: at every real field and index, the number a
   spectral port is republished at is at least that port's own, and nothing
   ...
   and may not publish a number the row's own certificate does not reach.
   The exact arm carries no number, so a reprice leaves it untouched. *)
...
(* A port's proposition at the program's own bound, together with an upper
   bound of that bound, is the port's proposition at the upper bound. The
   spectral arm's proposition is monotone in its number, and publishing an
   upper bound is what lets a row state the constant a paper cites while
   claiming about the coalition no more than the certificate proved. *)
...
(* The terminal publishing a row's accumulated bound as an upper bound of it.
   Post-processing of the published constant rather than a step: the data and
   the arms are unchanged, only the real the spectral arm's proposition
...
(* The name of the claim a published row carries, at one real field and one
   index of its family. The manifest row of a published row records the path
   and not the claim, so two rows over one model and one pair of statuses are
   one manifest row; this reader is where they differ, and it is decided by
   the port the row's certify statement wrote. *)
