...
(*                                                                            *)
(* Twelve positions 0..11. Three letter tables: reverse each four-card block (r4),    *)
...
(** psl211_r4_inj — the block-reversal table is an involution. *)
...
(** psl211_m6_permVE — the inverse of the half-Monge letter is the
...
Export (no `Local`) `psl211_wgenn`, `psl211_papply`, `psl211_word_perm`, `psl211_word_perm_mem`, `psl211_word_perm_val` (prefixed after the Task 3 quality review; `wapply` stays Local): Task 4 uses `psl211_papply` (the scalar application) and `psl211_word_perm`. pgl27 itself re-defines these locally in pgl27_orbit.v:549-595; here they are shared instead.

Adapt the pgl27 code with these substitutions and no others: the nat-level letter action `psl211_wgenn i` reads `psl211_r4_tbl` for `i = 0` and `psl211_m6_tbl` otherwise; `word_bfs` starts from `[:: 0; 1]` and iterates over `[:: 0; 1]` letters with fuel 20; `word_table_ok` quantifies over `iota 0 12` twice with the guard `a != b`; `wapply_map` is stated on pairs; `gen_of i` is `psl211_r4_perm` for `i = 0` and `psl211_m6_perm` otherwise; `gen_of_mem` has two cases; `pair_word x y : x != y -> exists w, psl211_papply w 0 = val x /\ psl211_papply w 1 = val y`.
...
Local Definition wstep (i : nat) (t : seq nat) : seq nat := map (psl211_wgenn i) t.
...
Qed.
Lemma psl211_word_perm_mem (w : seq nat) : psl211_word_perm w \in pgg_G psl211_M.
Proof.
...
Local Lemma pair_word (x y : 'I_12) :
  x != y -> exists w : seq nat, psl211_papply w 0 = val x /\ psl211_papply w 1 = val y.
Proof.
move=> nxy.
...
- [x] **Step 1: Deck vocabulary (pgl27_orbit.v lines 90-120 at twelve positions, six hearts)**
...
- [x] **Step 2: Invariance under the group (probe_orbit.v lines 144-310, verbatim, renamed)**
...
- [x] **Step 3: Action on decks (pgl27_orbit.v lines 289-334, twelve positions)**
...
- [x] **Step 4: Encoders and population**
...
- [x] **Step 5: Counts and orbit = family (L20, L21, L23, L25)**
...
(* As built (89aa93f): the `uniq tbl` premise is necessary, the plan's first
   statement was false for a table with a repeated row (tbl = [:: R; R],
   C = A = set0 gives 1 on the left and 2 on the right). psl211_orbit.v
   exports psl211_mirror_tbl_uniq and psl211_hexad_tbl_uniq (vm_compute) and
   psl211_asc6; the `all psl211_asc6 <table>` facts are Local there, so Task 7
   re-proves them by vm_compute where it needs them. *)
Proof.
...
Local Definition code_step (i : nat) (L : seq nat) : seq nat :=
  sort leq (map (psl211_wgenn i) L).   (* psl211_wgenn exported by psl211_group.v *)
...
Local Definition code_table_ok (b : bool) : bool :=
  all (fun L => has (fun sw : seq nat * seq nat =>
                       sort leq (map (psl211_papply sw.2) (psl211_rep_list b)) == L)
                    (code_table b))
...
(* subset_class_orbitE shape (pgl27_orbit.v:720-730): forward inclusion by
   ...
   (pgl27_orbit.v:643-653, restated over psl211_group.v's psl211_word_perm and
   psl211_papply) and list_to_setK. *)
Qed.
...
`psl211_wgenn`, `psl211_papply`, `psl211_word_perm`, `psl211_word_perm_mem`, `psl211_word_perm_val` are exported by Task 3 Step 2.

- [x] **Step 6: Compile and check**
...
(* card_orbit_stab (action.v:989): #|orbit| * #|stab| = #|G|; orbit = family
   ...
   (audit-plan/chk_fiber.v stab_card_shape, closed).  For
   psl211_pattern_countE supply `all psl211_asc6 tbl` (by vm_compute here)
   and `psl211_mirror_tbl_uniq` / `psl211_hexad_tbl_uniq` (psl211_orbit.v). *)
Qed.
...
- [x] **Step 1: In `psl211_group.v`, the closure and the group order (pgl27_mixing.v 72-166 and 239-557)**
...
- [x] **Step 2b: Deferred cosmetic fixes from the Task 4 re-review**

In `instances/psl211/psl211_orbit.v`: the two-subgoal `apply: gens_sub_stab` sites (about lines 264-267 and 274-277) use bullets; replace by two-space indentation as `stab_of_gen` does at 255-257. The `/and3P[_ H _]` at about 158 and 161 introduces a bare `H`; rename to `Hlt`. Recompile (70 s), then recompile psl211_scheme.v and every later psl211 file in order, commit as `style(psl211): indentation and hypothesis names in psl211_orbit.v`. From the Task 6 re-review: (R1) psl211_scheme.v:8-9 points at psl211_secrecy.v for the distributional statement; once Task 7 lands the pointer resolves, otherwise say "the design-privacy bridge"; (R2) hypothesis names `HHfam`, `HB'fam` in psl211_scheme.v become `HHsys`, `HB'sys`; (R3) wording: "quarter" at psl211_blocks.v:6 and :32 and every "four-card block" in psl211_group.v, psl211_blocks.v, the plan and the spec become "four-position segment", because "block" already names the six-position Steiner blocks in these files; (R4) `mem_ascending` -> `mem_ascending_lists` in psl211_blocks.v; (R5) probe the supported spelling of `enum S = [seq x <- enum 'I_12 | x \in S]` to drop `deprecated_filter_index_enum` (psl211_scheme.v:145, :163-165) and the `bigop` import; (R6) psl211_blocks.v:365 "generator" -> "enumerator". Also carried as a note: `psl211_mirror_tbl_uniq`, `psl211_hexad_tbl_uniq`, `tables_distinct(T)` are table certificates living in psl211_orbit.v rather than psl211_blocks.v (reviewer I5, left in place on purpose).

From the Task 10 review, tidy items: drop the unused imports of psl211_profile.v (`psl211_blocks`, `psl211_orbit`, `div`, `prime`, `primitive_action`, `covering_scheme`; keep `bigop`, `algebraic_rigidity`), and do the same inherited trim in pgl27_profile.v or in neither; say "chirality" once at the profile's secret-type comment instead of "orbit scheme"/"orbit-class plug"; extend the two thin comments on `psl211_se_exact` and `psl211_sw_bound`; re-check the forward reference to `psl211_word_mixing` at psl211_profile.v:97 once Task 9 lands.

- [x] **Step 2c: The Task 9 Step 1 quality pass, after psl211_mixing.v lands and before the full build**

Decided: split the closure and ptbl layer out of psl211_group.v into a new `instances/psl211/psl211_closure.v` (importing psl211_group; registered in _CoqProject right after psl211_group.v; psl211_mixing.v and psl211_secrecy.v import it; psl211_orbit.v and psl211_scheme.v do not), so the 94-second closure certificate leaves the orbit/scheme build path. In the same pass, in the moved region: rename `psl211_ptbl_sym -> psl211_ptbl_moves`, `psl211_mem_G_Ps -> psl211_mem_entry_perm`, `psl211_keys_size -> psl211_size_keys`, `psl211_uniq_elem_keys -> psl211_keys_uniq` (and the corresponding uses in psl211_mixing.v and psl211_secrecy.v); add `psl211_r4_permVE` (r4 is an involution, mirroring psl211_m6_permVE) in psl211_group.v and `psl211_inv_letterE (j : 'I_3) : (tnth psl211_moves j ^-1)%g = tnth psl211_moves (inord (psl211_inv_letter (val j)))` in the closure file; export `psl211_keys_closed` (the key-level closure fact) and `psl211_tbl_index_lt`, `psl211_tbl_index_key` (pgl27_mixing.v:718-736) if psl211_mixing.v did not already prove them; comment clauses on `psl211_tbl_index` (a non-key gets index 660), `psl211_entry_perm` (indices >= 660 name the identity, hence injectivity only on iota 0 660), `psl211_gen3_of` (inord clamps); rename the bare `H` in the six projection proofs; bullets on two subgoals at the gen3_eq proof; drop removable `@` where it compiles; header order and Key results; `have gen` -> `size_gen` / `ptbl_gen_fold`; one phrase for inverse-closure file-wide; the two bijection comments give the three-part account (lands in the group, onto, injective on iota 0 660). Correct the plan's as-built claim that `perm_inv_val` was omitted because milk6 is the only inverse letter (r4 is its own inverse and needs psl211_r4_permVE), and the Step 2 advisory to export `ptbl_r4` (unnecessary; the inverse fact is what Step 2 needs). Also in this pass: rename the exported `Wuni` of psl211_mixing.v to `psl211_Wuni` (pgl27_mixing.v exports a bare `Wuni`; with both imported the last import silently wins, confirmed by Locate) and update the plan's Step 3 statement text; export psl211_group.v's Local `keys_closed_mem` (character-identical statement) so psl211_mixing.v:263-296 (`key_of_G`, its `keys_closed_mem`) can be deleted; consider porting `pgl27_endpoint_mixing` / `pgl27_joint_mixing` with `From pgg_smc Require Import psl211_profile` (no cycle: psl211_profile does not import psl211_mixing). From the Task 7 quality review, also in this pass: export from psl211_orbit.v the enum-to-code-list lemmas (`sorted_val_enum`, `subseq_val_enum`, `all_lt12_enum`, `mem_sublists`, `count_ok_k_le5`) in the `enumT` spelling (secrecy's copies, not scheme's deprecated ones) and a specialised `psl211_pattern_transfer (C A : {set 'I_12}) : (0 < #|C|)%N -> (#|C| <= 5)%N -> A \subset C -> #|[set B in psl211_mirror_blocks | B :&: C == A]| = #|[set B in psl211_hexad_blocks | B :&: C == A]|`, so that scheme's `sys_pattern_transfer` and secrecy's `pattern_transferE` become thin wrappers (do not export sys_pattern_transfer: its statement mentions a Local); in psl211_secrecy.v: `fiber_card_total_action` -> `card_amove`-style name with the genericity note split from the statement comment, `fiber_inv_card` -> `fiber_inv_cardE`, `stab_card` -> `stab_cardE`, `set H :=` -> `set Hb :=`, `have E :` -> `have Hcases :`, `H1..H5` -> `Hk1..Hk5` (same in psl211_scheme.v), `H_of` -> `heart_block`/`orbit_H_blocksE` -> `orbit_blocksE`, an `Arguments` block pinning `secretP` explicit on the five exports and hiding inde_RV's trailing slots on the headline, move `psl211_leak_coalition` and `_card6` above the section, unspaced intro-pattern `|`, one line at the section divider saying why the combinatorial helpers sit outside the section. From the Task 9 quality review, also in this pass (highest leverage first): in the new psl211_closure.v define the table as a VALUE, `Definition psl211_elem_table : seq (seq nat * seq nat) := Eval vm_compute in elem_bfs 12 [:: (psl211_idt, [::])].` (measured: the representative mixing site drops from 7.3 s to 0.009 s and the walk-step chain from 94.6 s to 8.3 s; psl211_mixing.v projected from 173 s to about 20 s; the .vo grows to roughly 285 KB; `Local Opaque` in the consumer does nothing, refuted by measurement); move `perm_inv_val`, `gfwd_r4`, `ptbl_r4_inv`, `ptbl_inv_letter` (exported as `psl211_ptbl_inv_letter`) from psl211_mixing.v into psl211_closure.v next to psl211_ptbl_moves (gfwd_r4 is a byte-identical duplicate of the group file's; a future psl211_spectral.v needs ptbl_inv_letter); delete mixing's `keys_closed_mem` (25 lines, 6.4 s) and `key_of_G` in favour of the exported closure fact; rename `Wuni` -> `psl211_Wuni` (live collision: bare `Wuni` in pgl27_spectral.v:604,608 and pgl27_word_privacy.v:79 would resolve to psl211's once both are imported) and update the header box; header additions: the certificate is an inequality between binary naturals over the common denominator 660 * 3^584 with 3^584 the word count, the walk convention (weval is the right-ordered product, the reverse step is right multiplication by the inverse letter), and what is not here (closure table and checker in psl211_closure.v; endpoint and joint corollaries not ported); minors: `move=> H; exact: H` -> `exact: id` (test), drop redundant `by` before closing `exact:` (three sites), `exact:` at the final `apply:`, `psl211_ptbl_of_fwd (F := ...)` without `@`, unused binders `Hj`/`L'` -> `_`, tidy the Proof./Qed. split at ~257, drop dead imports `psl211_blocks` and `pgg_collusion_bound`, one spelling for the Monge letter ("Monge shuffle of each six-position half"). Rebuild the chain in order afterwards: blocks, group, closure, orbit, scheme, profile, secrecy, recovery, mixing.

- [x] **Step 3: Record deviations**
...
Follow-ups recorded from the Task 6 quality review (not blocking Plan A): `perm_of_eq_card` (equinumerous subsets exchanged by a permutation supported on their union) and `perm_onS` are mathcomp-level facts with no instance content; they move to a new mathcomp-only `lib/perm_exchange.v` in Plan B, restated in the `ubnP` form (no fuel parameter, no duplicated base case). Until then they stay Local in psl211_scheme.v with a source note. The header wording "learns nothing of the secret" (inherited from pgl27_scheme.v) is a set-level claim in the scheme file and a distributional one only in the secrecy file; the scheme comments say so.
...
As built, Task 7 (commit 534fab6): `psl211_colour_view_dep_k6` carries the premises `secretP true != 0 -> secretP false != 0 ->` (MATHEMATICAL: the plan's premise-free statement is false at a Dirac prior, where every observable is independent of the almost-surely-constant secret; both premises are minimal); the cardinality conjunct is also exported on its own as `psl211_leak_coalition_card6` so it is not trapped behind the premises; ledger L22's sharpness is therefore conditional on a non-degenerate prior, stated in the header. `psl211_colour_fiber_cardE` and `psl211_colour_view_indep` report the three boolp axioms because they sit in a section with `(R : realType) (secretP : R.-fdist bool)` variables (a proof-free Definition in the same section reports the same three; the Local combinatorial lemmas outside the section are closed). Route: view_fiberE (psl211_heart_set_act) -> fiber_inv_card (invg_inj, card_imset) -> fiber_partitionE (partition_big, amove_act, card_rcoset at 'P^*) -> stab_card (card_orbit_stab, psl211_card, the 132 counts) -> orbit_H_blocksE (the two _orbitE) -> pattern_transferE (psl211_count_okT; the empty coalition separately at 132 = 132). Scope order group then ring; psl211_scheme not imported (nothing used). The leak route is not the plan's sketch: instead of psl211_orbit_class_invariant it uses orbit_H_blocksE + mem_orbit (groupVr gG) + a cardinality squeeze to reach psl211_blocks_disjoint (same mathematics). The premised leak lemma strictly entails pgl27's premise-free shape at the uniform prior (spec review probe `uniform_leak_premise_free`). Duplicated helpers to collapse on the tidy list: `sorted_val_enum` (three copies: orbit, scheme, secrecy), `all_lt12_enum`, `subseq_val_enum`, `mem_sublists`, `count_ok_k_le5` (scheme and secrecy, byte-identical) and the nonempty branch of `pattern_transferE` (= the inner block of scheme's `sys_pattern_transfer`) -> export from psl211_orbit.v / psl211_scheme.v. Comment at psl211_secrecy.v:232-237 gives the wrong reason for not reusing sys_pattern_transfer (it is Local in an unimported file); fix in the quality pass.
...
As built, Task 11 (2026-09-15): Step 2b and 2c landed as four commits (4c08386 psl211_closure.v split with the renames psl211_ptbl_moves / psl211_mem_entry_perm / psl211_size_keys / psl211_keys_uniq and the new exports psl211_keys_closed, psl211_tbl_index_lt, psl211_tbl_index_key, psl211_ptbl_inv_letter, psl211_inv_letterE, psl211_r4_permVE; 98612df psl211_Wuni and the mixing header; d9406a2 the enumeration lemmas and psl211_pattern_transfer exported from psl211_orbit.v with scheme and secrecy as thin wrappers, the secrecy renames, the Step 2b orbit items; 6fda271 four-position-segment wording, enumerator, profile imports and comments). NOT done, with the measured reason: sealing psl211_elem_table as a value (`Eval vm_compute in`) made psl211_mixing.v run past 997 s at 4 GB against 158 s unsealed, because the kernel then compiles the written-out 660-row literal to bytecode at every vm_compute reaching the table; the projection in the Task 9 review was wrong and the closure header records the numbers. Per-file compile times at HEAD: blocks 38 s, group 4 s, closure 107 s, orbit 70 s, scheme 4 s, profile 4 s, secrecy 7 s, recovery 4 s, mixing 159 s (chain 397 s; flat in total, but orbit/scheme/profile no longer wait behind the closure certificate). Full build: `make -j8` at 6fda271 reports nothing to be done, exit 0 (every .vo current). Axiom sweep: check_axioms.sh per file (its directory mode needs bash 4 `mapfile`) reports "Standard axioms only" for all ten files; the nine anchors match REVIEWS.md; zero Admitted / Axiom under instances/psl211. Ledger rows closed by the landed code: L1-L9, L12-L23, L25 (L18 chirality by psl211_chirality_swapT; L19 by psl211_2transitive; L20 by psl211_card_*_blocks; L21/L25 by the two _orbitE; L22 by psl211_colour_fiber_cardE; L23 by psl211_pattern_countE); L10 dropped (mathcomp amove), L11 subsumed, L24 Plan B. Plan B (the executed cone: run, trace, exec, models, analysis, rows; the all-decks code view; the endpoint/joint mixing corollaries; lib/perm_exchange.v) is the next plan.

Step 2c review (Opus, four commits, 2026-09-15): GO, no blocking findings; every exported statement unchanged (multiset comparison of 315 -> 317 statements, independently confirmed by a second auditor), renames complete, layering as specified, the nine-anchor assumption table reproduced, hygiene clean. Its four comment findings and three nits landed in c7f46f8 (closure header states the measured 158 s -> >997 s regression and no longer asserts a bytecode-compilation mechanism, which the reviewer refuted at every shape measurable in isolation; mixing header points at psl211_closure.v; segment-reversal wording in group and closure; `Arguments psl211_count_ok_k_le5 [k]`), chain recompiled green in order (group 4 s, closure 99 s, orbit 75 s, scheme 4 s, profile 4 s, secrecy 7 s, recovery 4 s, mixing 159 s). Plan A final code HEAD: c7f46f8.

## Plan B as built (2026-09-15, branch feat/psl211-plan-b)

Landed: lib/perm_exchange.v (8c68c7c; `perm_onS`, `perm_of_eq_card` in ubnP form, no fuel, base case once; psl211_scheme.v imports it, the line-1589 follow-up is closed) and `psl211_endpoint_mixing`, `psl211_joint_mixing` with the Local `var_dist_prodR` at the end of Section psl211_mixing_sec (5032d4a; imports psl211_profile and pgg_collusion_bound, no cycle; the line-1601 follow-up is closed). Both reviewed (notes/probes/2026-09-15-psl211-planb/REVIEW-planb-mechanical.md): compliant, assumption sets as claimed, one blocking comment fix on the endpoint lemma's direction (`sigma s` is the slot position s receives) applied with the cosmetic lib fixes in the follow-up commit.

Probed, not landed (notes/probes/2026-09-15-psl211-planb/PROBE-REPORT.md): the `algebra { ... }` block accepts the psl211 names and `instance_profile psl211_algebra = psl211_profile` by `[]`; dealt-mode termination 0.45 s; `profile_endpoints_stmt psl211_algebra 220` closes by vm_compute in 562 s plus 331 s Qed at about 17 GB, flat in the fuel (220 and 380 equal), so the Observed level is reachable with no axioms and one 900-second declaration paid once at the profile. In dealt mode NEITHER Tableau arm can be certified: the raw code reading of C = {0,1,2} has a support difference between the two chiralities (reading (0,1,6) occurs under hexad and never under mirror), refuting `ew_indep` and `sc_const` at #|C| = 3 < 6. The all-decks parametrization `bool * 'I_132 * {perm 'I_6} * {perm 'I_6}` (136857600 points) typechecks through `supplied_input_params`, its termination closes in 0.42 s, its endpoints come from the profile fact through `supplied_endpointsE`, its reconstruction needs one validity claim `Hv`, and its `ExactWitness` demand is exactly ledger row L24, unproved. Decision pending: L24 in Rocq (new counting proof over the all-decks carrier), a colour-observer arm in the Tableau, or an Observed-level row without a certified port.
