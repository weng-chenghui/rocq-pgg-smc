...
(******************************************************************************)
(* The statement surface of a Verifolio program                                 *)
(*                                                                            *)
(* A program of pgg_verifolio.v is a sequence of binds over anonymous tuples.   *)
...
(* verifolio_bind, and the accumulated proposition is that of pgg_verifolio.v     *)
...
(* name, as Verifolio Observed in s5_verifolio_observed.v and Verifolio Sampled in  *)
(* s5_verifolio_sampled.v write it. Inside the publish position the three       *)
...
(* the reading. It is also the word of the bind of pgg_verifolio.v, following   *)
...
(* read. The bind of manifest/pgg_verifolio.v writes of ahead of a payload and  *)
...
Notation "A 'dealt' 'fuel' n" := (verifolio_start A ;;; dealt_step of n)
...
Definition params_step (x : StackAt Algebraic) (_ : StackProp Algebraic x)
    (E : ExecutionParams x) : Verifolio Executable :=
...
(* The algebra occurs in the types of the two arguments after it, which the
   ...
   it. The rules below pass the name unapplied to verifolio_bind and are
   unaffected. *)
...
Notation "t 'encoded' 'inputs' T 'layout' L 'by' enc 'decoded_by' d 'committed_by' procs 'fuel' n" :=
  (verifolio_start (tg_algebra t) ;;; params_step
...
Notation "A 'supplied' 'inputs' T 'layout' L 'expecting' e 'fuel' n" :=
  (verifolio_start A ;;; params_step of (supplied_input_params A T L e n))
  (at level 90, left associativity, T at level 10, L at level 10,
...
Notation "s 'execute' 'terminates' 'by' t 'endpoints' 'by' e 'recon' 'by' r" :=
  (s ;;; execute_step of (obs_payload (verifolio_at s) t e r))
...
Notation "s 'execute' 'terminates' 'by' 'vm_compute' 'endpoints' 'by' e 'recon' 'by' r" :=
  (s ;;; execute_step
       of (obs_payload (verifolio_at s) ltac:(by vm_compute) e r))
...
Notation "s 'certify' 'ExactIndependence' 'by' w 'leaks' 'at' k 'by' H" :=
  (s ;;; certify_exact of (exact_leaks (verifolio_at s) w k H))
  (at level 90, left associativity, w at level 0, k at level 0, H at level 0,
   only parsing).

Notation "s 'certify' 'InputIndistinguishability' 'by' c" :=
...
Notation "s 'certify' 'IdealProximity' 'by' c" :=
...
Notation "s 'certify' 'ExactIndependence' 'of' r 'by' w" :=
  (s ;;; certify_reading_exact
     of (reading_exact_payload (verifolio_at s) r w))
...
Notation "s 'certify' 'InputIndistinguishability' 'of' r 'by' c" :=
  (s ;;; certify_reading_indistinguishability
     of (reading_indistinguishability_payload (verifolio_at s) r c))
...
Notation "s 'certify' 'IdealProximity' 'of' r 'by' c" :=
  (s ;;; certify_reading_idealproximity
     of (reading_idealproximity_payload (verifolio_at s) r c))
  (at level 90, left associativity, r at level 0, c at level 0).
...
Notation "s 'certify' 'InputIndistinguishability' 'at' R idx 'by' b 'tied' 'by' Hd 'ideal' u 'mixing' 'by' Hc 'invariant' 'by' Hk" :=
  (s ;;; certify_indistinguishability of (mk_indistinguishability (verifolio_at s)
...
Notation "s |> 'publish' 'Obstruction' 'InputDistinguishability' 'of' r 'at' c 'by' pf 'assuming' a" :=
  (s ;;; publish_obstruction a
     of (mk_obstruction (verifolio_at s)
           (input_distinguishability_obstruction (verifolio_at s) r c) pf))
  (at level 90, left associativity, r at level 0, c at level 0,
