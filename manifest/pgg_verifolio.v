...
(******************************************************************************)
(* Verifolio: an analysis path of one protocol instance as a program            *)
(*                                                                            *)
...
(* nothing. And of the three terminals only conclude returns a verifolio and    *)
...
(*   VerifolioAt              == data at a level with a proof about it          *)
(*   verifolio_bind           == sequencing, written s ;;; f 'of' p             *)
...
(*                                                                            *)
(* Key results:                                                               *)
(*   verifolio_left_unit      == sequencing onto a built verifolio is application *)
...
Record VerifolioAt (b : CompletionLevel) (Q : StackAt b -> Prop) :=
  MkVerifolio {
    verifolio_at  : StackAt b ;
    verifolio_thm : Q verifolio_at }.

Arguments VerifolioAt : clear implicits.

(* A verifolio at the default proposition family of its level: what every line
   of a program returns until a terminal changes the family. *)
Notation Verifolio b := (VerifolioAt b (StackProp b)).
...
Definition verifolio_bind (a : CompletionLevel) (Q : StackAt a -> Prop)
    (P : StackAt a -> Type) (T : Type) (s : VerifolioAt a Q)
    (f : forall x : StackAt a, Q x -> P x -> T) (p : P (verifolio_at s)) : T :=
  f (verifolio_at s) (verifolio_thm s) p.
Arguments verifolio_bind {a Q P T} s f p.
...
Notation "s ;;; f 'of' p" := (verifolio_bind s f p)
...
Definition verifolio_start (A : PGGAlgebraic) : Verifolio Algebraic :=
  @MkVerifolio Algebraic (StackProp Algebraic) A I.

(* Sequencing a statement onto a verifolio built from given data and proof is that
...
Lemma verifolio_left_unit (a : CompletionLevel) (Q : StackAt a -> Prop)
    (P : StackAt a -> Type) (T : Type) (x : StackAt a) (pf : Q x)
    (f : forall y : StackAt a, Q y -> P y -> T) (p : P x) :
  verifolio_bind (@MkVerifolio a Q x pf) f p = f x pf p.
...
Definition dealt_step (x : StackAt Algebraic) (_ : StackProp Algebraic x)
    (fuel : nat) : Verifolio Executable :=
  @MkVerifolio Executable (StackProp Executable)
...
Definition execute_step (x : StackAt Executable) (_ : StackProp Executable x)
    (p : ObsPayload x) : Verifolio Observed :=
  @MkVerifolio Observed (StackProp Observed)
...
Definition sample_step (x : StackAt Observed) (q : StackProp Observed x)
    (f : FamPayload x) : Verifolio Sampled :=
  @MkVerifolio Sampled (StackProp Sampled)
...
(* The payload of the statement that names a reading: the reading, paired
   ...
   stack coordinate. *)
Definition ReadingExactPayload (x : StackAt Sampled) : Type :=
...
Definition reading_exact_payload (x : StackAt Sampled)
    (r : CoalitionReading (projT1 x))
    (w : forall (R : realType) (idx : amf_index (sp_f x) R),
           ExactWitness (amf_sample (sp_f x) R idx) r)
  : ReadingExactPayload x :=
  existT _ r w.
Arguments reading_exact_payload : clear implicits.
...
Definition ReadingIndistinguishabilityPayload (x : StackAt Sampled) : Type :=
  { r : CoalitionReading (projT1 x)
  & forall (R : realType) (idx : amf_index (sp_f x) R),
...
(* The pair the input-indistinguishability statement at a named reading
   ...
   written. *)
Definition reading_indistinguishability_payload (x : StackAt Sampled)
    (r : CoalitionReading (projT1 x))
    (c : forall (R : realType) (idx : amf_index (sp_f x) R),
           IndistinguishabilityCert (amf_sample (sp_f x) R idx) r)
  : ReadingIndistinguishabilityPayload x :=
  existT _ r c.
Arguments reading_indistinguishability_payload : clear implicits.
...
Definition ReadingIdealProximityPayload (x : StackAt Sampled) : Type :=
  { r : CoalitionReading (projT1 x)
  & forall (R : realType) (idx : amf_index (sp_f x) R),
...
Definition reading_idealproximity_payload (x : StackAt Sampled)
    (r : CoalitionReading (projT1 x))
    (c : forall (R : realType) (idx : amf_index (sp_f x) R),
           IdealProximityCert (amf_sample (sp_f x) R idx) r)
  : ReadingIdealProximityPayload x :=
  existT _ r c.
Arguments reading_idealproximity_payload : clear implicits.
...
Definition certify_exact (x : StackAt Sampled) (q : StackProp Sampled x)
    (p : ExactPayload x) : Verifolio AnalysisBridged :=
  @MkVerifolio AnalysisBridged (StackProp AnalysisBridged)
    (existT _ (projT1 x) (existT _ (projT1 (projT2 x))
...
Definition certify_reading_exact (x : StackAt Sampled)
    (q : StackProp Sampled x) (p : ReadingExactPayload x)
    : Verifolio AnalysisBridged :=
  @MkVerifolio AnalysisBridged (StackProp AnalysisBridged)
    (existT _ (projT1 x) (existT _ (projT1 (projT2 x))
       (existT _ (sp_Ht x) (existT _ (sp_He x) (existT _ (sp_Hr x)
          (existT _ (sp_f x)
...
Definition certify_indistinguishability (x : StackAt Sampled)
    ...
    (p : IndistinguishabilityPayload x) : Verifolio AnalysisBridged :=
  @MkVerifolio AnalysisBridged (StackProp AnalysisBridged)
    (existT _ (projT1 x) (existT _ (projT1 (projT2 x))
       (existT _ (sp_Ht x) (existT _ (sp_He x) (existT _ (sp_Hr x)
          (existT _ (sp_f x)
...
Definition certify_reading_indistinguishability (x : StackAt Sampled)
    (q : StackProp Sampled x)
    (p : ReadingIndistinguishabilityPayload x) : Verifolio AnalysisBridged :=
  @MkVerifolio AnalysisBridged (StackProp AnalysisBridged)
    (existT _ (projT1 x) (existT _ (projT1 (projT2 x))
       (existT _ (sp_Ht x) (existT _ (sp_He x) (existT _ (sp_Hr x)
          (existT _ (sp_f x)
...
Definition certify_reading_idealproximity (x : StackAt Sampled)
    (q : StackProp Sampled x) (p : ReadingIdealProximityPayload x)
    : Verifolio AnalysisBridged :=
  @MkVerifolio AnalysisBridged (StackProp AnalysisBridged)
    (existT _ (projT1 x) (existT _ (projT1 (projT2 x))
       (existT _ (sp_Ht x) (existT _ (sp_He x) (existT _ (sp_Hr x)
...
Definition conclude (c : ConcludedBound) (q : StackAt AnalysisBridged)
    ...
    : VerifolioAt AnalysisBridged (BridgedProp c) :=
  @MkVerifolio AnalysisBridged (BridgedProp c) q
    (conj (proj1 pf)
...
(* A program's data together with an arbitrary proposition proved from what the
   ...
   theorems. *)
Record RestatedVerifolio (Q : Prop) := MkRestatedVerifolio {
  rq_at  : StackAt AnalysisBridged ;
  rq_thm : Q }.
Arguments RestatedVerifolio : clear implicits.
...
Lemma certify_exact_propertyE (x : StackAt Sampled) (q : StackProp Sampled x)
    ...
    (idx : amf_index (ab_f (verifolio_at (@certify_exact x q p))) R) :
  ab_security_property (verifolio_at (@certify_exact x q p)) R idx
  = ExactIndependenceProperty.
Proof. by []. Qed.
...
Lemma certify_indistinguishability_propertyE (x : StackAt Sampled)
    ...
    (R : realType)
    (idx : amf_index
             (ab_f (verifolio_at (@certify_indistinguishability x q p))) R) :
  ab_security_property (verifolio_at (@certify_indistinguishability x q p)) R idx
  = InputIndistinguishabilityProperty.
Proof. by []. Qed.
...
Lemma certify_idealproximity_propertyE (x : StackAt Sampled)
    ...
    (idx : amf_index (ab_f (verifolio_at (@certify_idealproximity x q p))) R) :
  ab_security_property (verifolio_at (@certify_idealproximity x q p)) R idx
  = IdealProximityProperty.
Proof. by []. Qed.
...
Lemma certify_exact_readingE (x : StackAt Sampled) (q : StackProp Sampled x)
    (p : ExactPayload x) (R : realType)
    (idx : amf_index (ab_f (verifolio_at (@certify_exact x q p))) R) :
  ab_reading (verifolio_at (@certify_exact x q p)) R idx
  = coalition_endpoint_reading (projT1 x).
Proof. exact: erefl. Qed.
...
Lemma certify_indistinguishability_readingE (x : StackAt Sampled)
    ...
    (idx : amf_index
             (ab_f (verifolio_at (@certify_indistinguishability x q p))) R) :
  ab_reading (verifolio_at (@certify_indistinguishability x q p)) R idx
  = coalition_endpoint_reading (projT1 x).
Proof. exact: erefl. Qed.
...
Lemma certify_idealproximity_readingE (x : StackAt Sampled)
    (q : StackProp Sampled x) (p : IdealProximityPayload x) (R : realType)
    (idx : amf_index
             (ab_f (verifolio_at (@certify_idealproximity x q p))) R) :
  ab_reading (verifolio_at (@certify_idealproximity x q p)) R idx
  = coalition_endpoint_reading (projT1 x).
Proof. exact: erefl. Qed.
...
Lemma conclude_propertyE (c : ConcludedBound) (q : StackAt AnalysisBridged)
    (pf : StackProp AnalysisBridged q) (p : ConcludePayload c q)
    (R : realType)
    (idx : amf_index (ab_f (verifolio_at (conclude c q pf p))) R) :
  ab_security_property (verifolio_at (conclude c q pf p)) R idx
