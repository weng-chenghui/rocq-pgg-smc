...
(* - Smallest faithful permutation degree N = 97,239,461,142,009,186,000     *)
(*   (~ 9.7 * 10^19), the index of the largest maximal subgroup 2.B          *)
(*   [ATLAS of Finite Groups, Conway-Curtis-Norton-Parker-Wilson 1985;       *)
(*   online ATLAS v3, page M].                                                *)
(* - 2-generated: every finite simple group is 2-generated                    *)
(*   [Aschbacher-Guralnick 1984, J. Algebra 90:446-460, via the               *)
(*   classification]; for M an explicit pair with a^2 = b^3 = (ab)^7 = 1     *)
(*   is computed in [Wilson 2001, J. Group Theory 4:367-374].                 *)
(* - The Monster is far too large to enumerate computationally                *)
(*                                                                            *)
(* PGG implications:                                                          *)
(* - Security at L*: the two L* axioms below force 2^L* <= N (the search     *)
(*   space has exactly 2^L* elements and injects into the N card positions), *)
(*   so at the ATLAS degree L* <= 66 and the direct bound                     *)
(*   epsilon = 2*(N - 2^Lstar)/N is at least 2*(N - 2^66)/N ~ 0.48.  N is    *)
(*   not a power of two, so epsilon = 0 is unreachable; an earlier reading    *)
(*   of this file with L* = 67 and epsilon = 0 contradicts its own axioms.    *)
...
(*   monster_sigmas : two generators (Aschbacher-Guralnick 1984; Wilson 2001) *)
(*   monster_sigmas_distinct : generators are distinct permutations          *)
(*   monster_Lstar  : word length at which the bound is read (<= 66 at the   *)
(*                    ATLAS degree, forced by the two axioms below)          *)
...
(*     (via security_witness_endpoint_inj, eps = 2(N-2^Lstar)/N >= 0.48)    *)
...
(* monster_n.+2 = number of card positions in the smallest faithful permutation
   representation of the Monster group: the action on the cosets of the
   largest maximal subgroup 2.B, of index 97,239,461,142,009,186,000
   (~ 9.7 * 10^19).  Source: Conway, Curtis, Norton, Parker, Wilson, ATLAS
   of Finite Groups, OUP 1985 (Monster page); online ATLAS of Finite Group
   Representations v3, page M, maximal subgroup 2.B and its index. *)
Axiom monster_n : nat.

(* Two generators.  Every finite simple group is generated by two elements:
   Aschbacher and Guralnick, "Some applications of the first cohomology
   group", J. Algebra 90 (1984) 446-460, resting on the classification.  For
   the Monster an explicit generating pair a, b with a^2 = b^3 = (ab)^7 = 1
   (classes 2B, 3B, 7B) is computed in R. A. Wilson, "The Monster is a
   Hurwitz group", J. Group Theory 4 (2001) 367-374; the ATLAS standard
   generators are a in 2A, b in 3B with ab of order 29. *)
...
(******************************************************************************)
(*     L* axioms: the word length at which the direct bound is read          *)
(*                                                                            *)
(* For the Monster with N = 97,239,461,142,009,186,000 and Tg = 2:          *)
(*   2^66 = 73,786,976,294,838,206,464  < N                                  *)
(*   2^67 = 147,573,952,589,676,412,928 > N                                  *)
(* The two axioms below jointly force 2^L* <= N: word-eval injectivity      *)
(* makes the search space at L* exactly 2^L* (weval_inj_search_space), and   *)
(* endpoint injectivity embeds that set into the N card positions            *)
(* (card_in_imset, max_card).  Hence L* <= 66 here, and the direct endpoint  *)
(* epsilon = 2*(N - 2^Lstar)/N is at least 2*(N - 2^66)/N ~ 0.48. It cannot  *)
(* be 0, because N is not a power of two.  The subtraction in direct_eps is  *)
(* natural-number subtraction, so a reading with 2^L* > N would show         *)
(* epsilon = 0 only by truncation, and is excluded by the axioms anyway.     *)
(******************************************************************************)

(* The word length at which the direct endpoint bound is read.  Kept
   abstract so that the epsilon below is a formula in it rather than a
   decimal.  No literature source: it is a modelling parameter, constrained
   only by the two axioms below, which force 2^L* <= N (L* <= 66 at the
   ATLAS degree). *)
Axiom monster_Lstar : nat.

(* Distinct words of length L* evaluate to distinct group elements, so the
   search space at L* is the full 2^L*.  No literature source: this is the
   statement that the two chosen generators satisfy no relation between
   positive words of length L*, a property of the specific generating pair
   that no published result about the Monster asserts.  The cardinality
   comparison 2^L* <= N is necessary for the next axiom, not sufficient for
   this one. *)
...
(* Endpoint evaluation injective on achievable(L_star): for each start card,
   ...
   permutations at L*.  No literature source: it is a modelling assumption
   about the chosen generators.  Together with monster_weval_inj_Lstar it
   forces 2^L* <= N (a set of 2^L* permutations injects into N positions),
   which is the constraint recorded in the section header. *)
...
(* The security half of the rigidity pair: for every card position, the
   ...
   2*(N - 2^Lstar)/N of uniform in the full-L1 convention.  At the ATLAS
   degree the axioms allow L* at most 66, so that epsilon is at least
   2*(N - 2^66)/N ~ 0.48 and the endpoint marginal is not uniform; the
   data-processing route at length 1 gives 2*(N! - 2)/N!, which is nearly 2
   and says even less.  The
...
(* The Monster admits a covering scheme, the geometric object the threshold
   ...
   the same object from the Reed-Solomon construction instead.  The geometric
   half is grounded: M is a Hurwitz group (R. A. Wilson, "The Monster is a
   Hurwitz group", J. Group Theory 4 (2001) 367-374), so M is the deck group
   of a Galois cover of P^1 branched over three points with ramification
   orders 2, 3, 7, of genus 1 + |M|/84 by Riemann-Hurwitz.  The threshold
   scheme packaged alongside it is a modelling choice with no source. *)
...
(* If this particular covering has genus 0 then the group obeys Klein's
   ...
   Source for the bound: F. Klein, Vorlesungen ueber das Ikosaeder und die
   Aufloesung der Gleichungen vom fuenften Grade, Teubner, Leipzig, 1884
   (classification of the finite subgroups of PGL(2, C): cyclic, dihedral,
   A_4, S_4, A_5); the bound itself is klein_genus0_bound in
   cover_tradeoff.v. *)
...
(** monster_rigidity — one algebraic choice, the Monster acting on its
    ...
    endpoint marginal bound at L*, whose epsilon is 2*(N - 2^Lstar)/N (at least
    about 0.48 under the axioms), and the threshold witness, whose covering
    genus grows with the group order.  The instance is the extreme case of
    the coupling the record exists to expose: the group with the largest
    search space is the group that makes the threshold half worst.  No exact
    or asymptotic mixing certificate is attached. *)
...
(* The covering falls on one of two sides: at genus 0 the group obeys the
   ...
   the gap it allows is the price the Monster pays for its large search
   space. *)
