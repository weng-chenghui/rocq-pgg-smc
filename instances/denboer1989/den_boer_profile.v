...
(** den_boer_eps0_lt — the eps = 0 upper-positivity constraint of the family:
    0 < 1/5, discharging the family's `eps < 1/5` hypothesis at the den Boer
    instance's bias. *)
...
(** den_boer_eps0_gt — the eps = 0 lower-positivity constraint of the family:
    -(4/5) < 0, discharging the family's `-4/5 < eps` hypothesis at the den
    Boer instance's bias. *)
Lemma den_boer_eps0_gt (R : realType) : - (4%:R * 5%:R^-1) < (0 : R).
Proof. by rewrite oppr_lt0 divr_gt0 // ltr0n. Qed.

(** den_boer_eps0_spectral — the eps = 0 spectral-gap constraint of the
    family: `|0| = 0 < 4/5, discharging the family's spectral-gap hypothesis
    `|eps| < 4/5 at the den Boer instance's bias. *)
Lemma den_boer_eps0_spectral (R : realType) : `|(0 : R)| < 4%:R / 5%:R.
Proof. by rewrite normr0 divr_gt0 // ltr0n. Qed.

(** den_boer_profile — den Boer is the unbiased five-card family member: the
    five-card MonodromyProfile bundling FiveCardKim_M, the secret type bool,
    FiveCardKim_PI and the bool/'I_5 five-card plug five_card_plug. This is
    the foundational five-card trick read off the shared five-card family; the
    bias and word length that distinguish the den Boer member from the Kim
    family are data of the marginal bound below, not of the program layer. *)
...
(** den_boer_marginal_bound — the marginal bound of the unbiased one-cut
    five-card member: the bound projection of the five-card certificate bundle
    at bias 0 and word length 1. *)
...
(** den_boer_perfect — the den Boer dealing-phase security bound is exactly 0:
    sw_bound_eps den_boer_marginal_bound = 0. This is the precise sense in
...
(** profile_k_denboer — the five-card plug's privacy threshold is 2:
    profile_k den_boer_profile = 2. This is the contrast character read off
    the shared profile_k of the five-card plug: any single revealed card
    leaks nothing about the AND, but two may. *)
...
(** FiveCardKim_Teq — the scheme and interface party counts agree (both 4).
    This is the cast witness, kept as erefl so tuple casts reduce away; it
...
(** FiveCardKim_G_stable — the monodromy permutes the starts as the share
    permutation, in the content = fc_content = id form. This is the structural
...
(** FiveCardKim_protocol_correct — recovery of the dealt endpoints returns the
    secret bit, unconditionally, on the five-card family instance. This is the
    end-to-end guarantee for the five-card trick on FiveCardKim_M: for any
    hidden element P of the full C_5 monodromy, reconstructing the revealed
    endpoints recovers the secret bit, via the generic
    pgg_recon_monodromy_correct fed FiveCardKim_G_stable and the family's
    full-group reconstruction invariance fcI_perm_compatible_kim. This is the
    den Boer member's correctness theorem for the five-card family. *)
...
(** den_boer_players — the five-player list for the den Boer dealing phase:
    the explicit five-element list of 'I_5 player ordinals. A concrete list,
    rather than enum 'I_5, is what lets the dealer's fold_senv reduce under
    vm_compute when checking session-type duality. *)
...
(** den_boer_dealer_committed — the den Boer dealer with the M = 2
    input-commitment prologue: exchange_dealer_with_commit at FiveCardKim_PI
    receiving from input parties 7 and 8, dealing the identity word, then
    running the fc_content dealing body for the five players. This routes the
    foundational five-card trick through the shared committed-dealer program
    with the two input bits committed up front. *)
...
(** den_boer_dealer_committed_ap — the committed den Boer dealer as an aproc,
    packaged for the duality checks. *)
...
(** den_boer_input0_ap — input party 0 (process id 7) committing bit a as an
    aproc, packaged for the duality checks. *)
...
(** den_boer_player0_ap — den Boer player 0 as an aproc, packaged for the
    duality checks. *)
Definition den_boer_player0_ap :=
  mk_aproc (exchange_player FiveCardKim_PI (@Ordinal 5 0 isT)).

(** den_boer_verifier_ap — the den Boer verifier as an aproc, packaged for the
    duality checks. *)
Definition den_boer_verifier_ap :=
  mk_aproc (exchange_verifier FiveCardKim_PI den_boer_players).

(** den_boer_commit_input0_dual — the committed dealer is dual to input party
    0. The prologue's first receive is the session dual of the first input
    party's bit commit, a embedded via encode_bool, for the concrete M = 2 den
    Boer instance. *)
...
(** den_boer_commit_input1_dual — the committed dealer is dual to input party
    1. The prologue's second receive is the session dual of the second input
    party's bit commit, b embedded via encode_bool, for the concrete M = 2 den
    Boer instance. *)
Lemma den_boer_commit_input1_dual (b : bool) (P_idx : nat) :
  channels_dual (den_boer_dealer_committed_ap P_idx) (den_boer_input1_ap b).
Proof. apply/eqP. rewrite /channels_dual /are_dual. by vm_compute. Qed.

(** den_boer_commit_player0_dual — the committed dealer stays dual to player
    0. The input-commitment prologue does not disturb the dealing-phase
...
(** den_boer_commit_verifier_dual — the committed dealer stays dual to the
    verifier. The input-commitment prologue does not disturb the
...
(** den_boer_committed_nil — with no committed inputs, the committed dealer is
    the plain den Boer dealer on the identity word table. This is the M = 0
    degeneration specialised to the den Boer instance, holding by
    computation: it confirms the committed dealer extends, rather than
    replaces, the uncommitted dealing program. *)
...
(** den_boer_committed_protocol_correct — end-to-end correctness through the
    committed dealer: for any hidden monodromy element P, reconstructing the
    dealt endpoints recovers the secret bit. The reconstruction is fixed by
    the starting layout and is invariant under the monodromy, since
    pgg_recon_endpoints does not depend on the dealer's word table, so
    committing the two input bits up front leaves correctness exactly as
    FiveCardKim_protocol_correct. *)
