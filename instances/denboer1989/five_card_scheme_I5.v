...
(** fcI_valid — shares (card positions in 'I_5) encode secret bit s iff the
    decoded heart-pattern has three consecutive hearts equal to s. This is the
    validity predicate of the bool/'I_5 threshold scheme. *)
Definition fcI_valid (s : bool) (shares : 5.-tuple 'I_5) : Prop :=
  fc_three_consec [seq decode_bool x | x <- shares] = s.

(** fcI_recon — recover the secret bit from 'I_5 shares: decode each position,
    then read three consecutive hearts. This is the reconstruction map of the
    bool/'I_5 threshold scheme. *)
Definition fcI_recon (shares : 5.-tuple 'I_5) : bool :=
  fc_three_consec [seq decode_bool x | x <- shares].

(** fcI_encode — canonical 'I_5 share row for a secret bit: the canonical bool
    arrangement fc_arrange_tup s s encoded into positions via encode_bool.
    This is the encoding map of the bool/'I_5 threshold scheme. *)
Definition fcI_encode (s : bool) : 5.-tuple 'I_5 :=
  [tuple of [seq encode_bool x | x <- fc_arrange_tup s s]].

(** fcI_reconK — reconstruction of valid shares returns the secret. This is
    the ts_correct field of fcI_scheme: fcI_recon and fcI_valid are the same
    fc_three_consec expression, so validity is exactly the recovered value. *)
Lemma fcI_reconK (s : bool) (shares : 5.-tuple 'I_5) :
  fcI_valid s shares -> fcI_recon shares = s.
Proof. by rewrite /fcI_recon /fcI_valid => ->. Qed.

(** fcI_encode_valid — the canonical encoding is valid for its secret. This is
    the ts_encode_valid field of fcI_scheme. *)
Lemma fcI_encode_valid s : fcI_valid s (fcI_encode s).
Proof.
(* The codec round-trips (decode_encode_bool), reducing to
   fc_three_consec (fc_arrange s s) = s, which is fc_correct at shift 0
   (rot 0 = id) with s && s = s. *)
...
(** fcI_wit — a bool heart-pattern, valid for secret s, with a chosen value at
    a chosen position i, so a single revealed card leaks nothing. This is the
    privacy witness reused at the 'I_5 level: each individual card position is
    compatible with both AND = 0 and AND = 1. *)
...
(** fcI_wit_size — every branch of fcI_wit returns a five-element seq. This is
    the size proof needed to wrap fcI_wit into a 5.-tuple. *)
Lemma fcI_wit_size (s : bool) (i : nat) (v : bool) :
  i < 5 -> size (fcI_wit s i v) == 5.
Proof. by case: s; case: v; case: i => [|[|[|[|[|]]]]]. Qed.

(** fcI_wit_tup — the privacy witness as a 5-tuple of bools, lifting fcI_wit
    to the tuple shape fc_three_consec expects. *)
Definition fcI_wit_tup (s : bool) (i : 'I_5) (v : bool) : 5.-tuple bool :=
  Tuple (@fcI_wit_size s (val i) v (ltn_ord i)).

(** fcI_wit_valid — the witness tuple reconstructs the secret s, so
    fcI_wit_tup is valid for s and can serve as a privacy witness. *)
Lemma fcI_wit_valid s (i : 'I_5) v :
  fc_three_consec (val (fcI_wit_tup s i v)) = s.
Proof. by case: s; case: v; case: i => [[|[|[|[|[|]]]]]] //. Qed.

(** fcI_wit_tnth — the witness tuple carries value v at position i, letting
    the privacy proof pin one position to the actual share value while keeping
    the reconstructed secret fixed. *)
...
(** fcI_private — any single card position (|C| < 2) can be matched by shares
    valid for a different secret. This is the ts_private field of fcI_scheme:
    the single-card-position privacy property of the threshold scheme. *)
...
Proof.
(* For the singleton coalition {j}, build an 'I_5 share row that carries the
   actual revealed value at j, so the coalition view is identical, while
   decoding to a bool witness valid for the other secret elsewhere.
   encode_bool injectivity (via decode_encode_bool) transports the bool
   witness's validity through the codec. *)
case Hs12: (s1 == s2).
...
(** fcI_scheme — the five-card threshold scheme with a boolean secret and 'I_5
    (card-position) shares: 5 shares, privacy threshold 2, so any single card
    position reveals nothing about the AND. This is the heterogeneous-secret
    scheme plugged into the Kim-family reconstruction plug (five_card_plug,
    five_card_family.v); taking secretT = bool is what avoids the surjectivity
    contradiction that blocks an 'I_5 secret. *)
...
(** fc_sigma_pow_val — the k-th power of the 5-cycle advances a position by k
    modulo 5. This is what turns the monodromy reindexing
    tnth shares ((fc_sigma^k) i) into a cyclic rotation of the decoded share
    row. *)
...
(** fc_three_consec_rot — three consecutive hearts is invariant under a cyclic
    shift by k < 5 of a five-card row. This is the rotation-invariance fact
    that lets the fc_sigma monodromy reindexing pass through fc_three_consec
    unchanged. The hypothesis k < 5 is essential: MathComp's rot j s returns s
    unchanged when j >= size s, so only genuine shifts (k mod 5 < 5) actually
    rotate the row. *)
