...
(** pgl27_rho_dist — the uniform distribution over the PGL(2,7) shuffle group.
    The shuffle law of the eight-card orbit scheme. *)
...
(** pgl27_sw_bound — the single-card pushforward meets the epsilon = 0
    bound. *)
...
(** pgl27_certificate_bundle — the marginal bound above with the
    exact-equality certificate attached and no asymptotic certificate.  The
    exact slot carries the closed-form equality, distance zero rather than a
    bound, so nothing about this instance rests on an asymptotic argument. *)
...
(** pgl27_profile — the PGL(2,7) plug of the shared MonodromyProfile: the
    group, the secret type, PI and the orbit plug. The eight-card orbit-class
    plug of the MonodromyProfile program. *)
...
(** profile_k_pgl27 — the PGL(2,7) plug's privacy threshold is four.
    Coalitions of at most three cards are private, k = 4. *)
Lemma profile_k_pgl27 : profile_k pgl27_profile = 4.
Proof. by []. Qed.

(** profile_eps_pgl27 — the PGL(2,7) marginal bound's epsilon is zero.
    The epsilon field of pgl27_marginal_bound is 0: the single-card
