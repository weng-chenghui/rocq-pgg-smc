...
(* on the first two lines, the three run facts on the third, and one security *)
(* record on the fourth. The security mathematics reaches a row through that  *)
(* last payload alone, and through three named facts: pgl27_static_obsE,      *)
...
(*   pgl27_exact_four      == the exact arm's four conjuncts at this instance *)
...
Definition pgl27_dealt : ViewLift Observed :=
  lift_start pgl27_algebra
    ...
    ;;; execute_step of (existT _ pgl27_terminates
                           (existT _ pgl27_endpoints pgl27_recon)).
...
(* The exact arm's witness: the dealt secret as a random variable on the exact
   ...
   the reading carry no information about the secret at all, not a small amount.
   The framework derives the zero mutual information, the unchanged
...
(* At fewer than four colluding seats the executed coalition reading is
   independent of the dealt secret, carries zero mutual information with it,
   leaves the secret's entropy unchanged under conditioning, and stays
   independent of it under every deterministic function of the seat-to-card
   map. The four conjuncts are the whole content of the exact arm at this
   instance; the proof is the row's security projection applied, so a reader
   who wants the information-theoretic reading of the row needs no further
   derivation. *)
Theorem pgl27_exact_four (R : realType) (C : {set 'I_8})
    (HC : (#|C| < 4)%N) :
...
(* The two arms are different claims and the projections do not interchange.
   A row certifying decay is asked here for the exact arm's conclusion at a
   coalition and a threshold proof, and is rejected: what the word row proves
   at a coalition is a distance between the readings of two dealt secrets, so it
   takes the two secrets before the threshold proof. *)
