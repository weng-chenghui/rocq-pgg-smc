...
(** The two deck pairs hold their hearts at the same positions for each secret
    value. The decoder reads only the heart four-subset, so this is what makes
    the two pairs two encodings of one secret rather than two secrets. *)
...
(** The two deck pairs decode to the same secret. A decoder that reads the
    heart positions returns the dealt secret on either pair, so a coalition
    holding enough positions recovers the same bit whichever pair the dealer
    uses, and the leakage values below compare like with like. *)
...
(** Both pairs leak nothing to a coalition of at most three positions. Three
    is the privacy threshold of the eight-card scheme, and it comes from sharp
    3-transitivity of the shuffle group together with distinctness of the
    cards, neither of which the choice of decks can move. *)
...
(** Both pairs leak eleven fourteenths of a bit to an equianharmonic
    four-position coalition. The two pairs differ by a transposition of two
    cards, and at the equianharmonic class that transposition leaves the
    number of shuffles producing one view under both decks unchanged. *)
...
(** At a harmonic four-position coalition one pair leaks five sevenths of a
    bit and the other six sevenths. This is the smallest coalition size at
    which the choice of decks is visible at all, and it is also where the two
    pairs put the minimum of their leakage on opposite cross-ratio classes. *)
...
(** At five positions one pair leaks twenty-five twenty-eighths of a bit and
    the other the whole bit. Five positions already determine the secret at
    one pair and leave it undetermined on some executions at the other, which
    is the gap between the two recovery thresholds. *)
...
(** At six positions one pair leaks twenty-seven twenty-eighths of a bit and
    the other the whole bit. Six is the second and last size inside the gap
...
(** The recovery thresholds of the two pairs are seven and five. Both pairs
    have the privacy threshold three, so the choice of decks moves only the
    size at which a coalition recovers the secret, and moves it by two
    positions out of eight. *)
