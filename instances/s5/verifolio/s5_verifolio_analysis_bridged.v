...
(******************************************************************************)
(* s5_verifolio_analysis_bridged: the five-seat instance at AnalysisBridged     *)
(*                                                                            *)
...
(* to it modulo five, one share per seat, and each seat reads the card at its *)
(* own position, the cut of this model being the identity. Five is the        *)
(* threshold the derived profile declares, so every statement here that       *)
(* quantifies over a coalition quantifies over at most four of the five       *)
...
(* statement of the instance enters the program.                              *)
(*                                                                            *)
...
(* equation, s5_verifolio_sampled for a named model, s5_verifolio_observed for a  *)
(* run or a specification, and s5_verifolio_executable for a parameter record.  *)
...
(** The framework's direct computation of a coalition's view, as a random
    ...
    security statement of a program is made about the left-hand side and every
...
(** Below five seats, the framework's direct computation of a coalition's view
    is independent of the tape secret. It is s5_exec_coalition_secrecy, the
    additive sharing's privacy at a uniform tape, carried along the
    identification above and read from zero mutual information back to
    independence. This is the whole of the mathematics the supplied program's
...
(** The randomized program: the supplied run above, the uniform tape model, the
    witness above, and the manifest path. Its last statement publishes a path
    whose transfer status is StaticExecutedOnly, because the model's cut is the
    identity and no idealized shuffle is being compared with a real one, and
    whose assumption status names the instance's group-order fact, which enters
    through the reconstruction plug the algebra's profile carries. What the
    finished program carries about a coalition of fewer than five seats is
    independence of the tape secret, at every real field, with no numeric bound
    anywhere in it. *)
...
(** The path the program publishes is the manifest's own path for this instance
    under the uniform tape. Conversion decides it, so the descriptive path and
...
(** The program is the named Sampled value with the payload and the terminal
    ...
    the Sampled file names the value they build, so this equation is what keeps
    the two spellings of the program's prefix from parting. *)
