...
(******************************************************************************)
(* psl211_verifolio_algebraic: the twelve-card instance at the Algebraic level  *)
(*                                                                            *)
...
(* model. Every twelve-card program begins at that algebra, and the           *)
(* instance's two analyses first differ three levels above, at Sampled, where *)
...
(** The twelve-card chirality instance at the Algebraic level: the algebra
    alone, under True, the proposition that level carries. One run mode is built
    on this value, the all-decks one, and psl211_alldecks_executableE is where
    the prefix both published programs continue from is identified with it. The
    dealer-dealt parameters of psl211_exec.v are built from the same algebra and
    from no program. What separates the run modes is the level above and not
    this one. *)
Definition psl211_algebraic_start : Verifolio Algebraic :=
  verifolio_start psl211_algebra.
