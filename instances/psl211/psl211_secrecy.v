...
Local Lemma orbit_H_blocksE (b : bool) :
  orbit 'P^* (pgg_G psl211_M) (H_of b)
...
Local Lemma colour_heartE (b : bool) (g : pgg_gT psl211_M) (i : 'I_12) :
  ...
  = (i \in (g^-1)%g @: H_of b).
Proof. by rewrite /H_of -psl211_heart_set_act inE tnth_mktuple. Qed.

(* The elements of G carrying x to a point of its orbit form one right coset
   of the stabiliser (mathcomp amove_act and card_rcoset), stated at a total
   action so that it instantiates at 'P^*.  No psl211 content: candidate for
   a mathcomp-only lib file, kept Local here until one exists, as
   psl211_scheme.v does for perm_of_eq_card. *)
Local Lemma fiber_card_total_action (gT : finGroupType) (rT : finType)
...
Proof.
have -> : [set g in pgg_G psl211_M | ((g^-1)%g @: H_of b) :&: C == A]
  = [set (g^-1)%g
    | g in [set g in pgg_G psl211_M | (g @: H_of b) :&: C == A]].
...
rewrite [LHS](partition_big (fun g : pgg_gT psl211_M => [set g x | x in H])
  (fun B => (B \in orbit 'P^* G H) && (B :&: C == A))); last first.
  by move=> g /andP[gG Hg]; rewrite Hg andbT; exact: mem_orbit.
rewrite (eq_bigr (fun _ => s)); last first.
  move=> j /andP[Hj HjA].
  rewrite (eq_bigl (fun i => (i \in G) && ([set i x | x in H] == j)));
    last first.
    move=> i; case Hij: ([set i x | x in H] == j); last by rewrite !andbF.
    by rewrite (eqP Hij) HjA !andbT.
  by rewrite sum1dep_card; exact: (fiber_card_total_action Hj).
...
(* The block-count certificate is a conjunction over the five coalition
   sizes; this is its projection at one size.  The false branches are
   discharged in a context holding only numbers, since deciding a hypothesis
   of the form psl211_count_ok_k i by computation does not terminate in
   usable time. *)
...
(* -------------------------------------------------------------------------- *)
(* The sample space, the colour observer and the privacy statements.          *)
(* -------------------------------------------------------------------------- *)
...
rewrite (fiber_inv_card C (pattern_of C v) true)
        (fiber_inv_card C (pattern_of C v) false).
rewrite (fiber_partitionE C (pattern_of C v) true)
        (fiber_partitionE C (pattern_of C v) false).
rewrite (stab_card true) (stab_card false) !orbit_H_blocksE /=.
...
Proof.
move=> Hpt Hpf.
have HCeq : psl211_leak_coalition = H_of true by [].
...
have Hhexo : orbit 'P^* (pgg_G psl211_M) (H_of false) = psl211_hexad_blocks.
  by rewrite orbit_H_blocksE.
have Hmiro : orbit 'P^* (pgg_G psl211_M) (H_of true) = psl211_mirror_blocks.
  by rewrite orbit_H_blocksE.
have Hview_false : forall g, g \in pgg_G psl211_M ->
    psl211_colour_view psl211_leak_coalition (false, g) <> v0.
  move=> g gG Hveq.
  have Hsub : psl211_leak_coalition \subset (g^-1)%g @: H_of false.
    apply/subsetP => i Hi.
  ...
  have Heq : (g^-1)%g @: H_of false = psl211_leak_coalition.
