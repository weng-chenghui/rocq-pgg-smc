...
(*                                                                            *)
(*   psl211_r4_perm == reverse each of the three quarters 0..3, 4..7, 8..11   *)
...
(*   word_perm w == a word w over the two letter indices, folded into the     *)
(*                  composite shuffle permutation it names                    *)
(*                                                                            *)
(* Key results:                                                               *)
...
(*                         positions                                          *)
(*                                                                            *)
(* 2-transitivity is consumed by the ten-reveal ambiguity transport           *)
(* (psl211_recovery.v) and by the single-card uniformity of the profile. The  *)
...
(** psl211_r4_perm — the quarter-reversal letter as a permutation of the
    twelve positions. *)
...
(** psl211_r4_permE — the quarter-reversal permutation acts on a position by
    table lookup. *)
...
(* A word is a seq of letter indices (0 = quarter reversal, 1 = half Monge).  *)
...
(** wgenn i — the nat-level action of letter i on a card position, read off
    the quarter-reversal table for i = 0 and the half-Monge table otherwise. *)
Definition wgenn (i : nat) : nat -> nat :=
  if i == 0 then (fun a => nth 0 psl211_r4_tbl a)
  else (fun a => nth 0 psl211_m6_tbl a).

(** papply w a — the position reached from a by applying the letters of the
...
(** word_perm w — the word w folded into the composite shuffle permutation
...
(** word_perm_mem — every composite word permutation lies in the shuffle
    group. *)
...
(** word_perm_val — the composite word permutation agrees with nat-level word
