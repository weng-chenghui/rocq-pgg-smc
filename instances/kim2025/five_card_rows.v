...
(* program publishes that manifest row through the input-indistinguishability *)
(* arm, and what the arm certifies is this: for each real field, for every    *)
(* coalition of at most one of the five seats and for every two committed     *)
(* pairs, the law of that coalition's static endpoint reading under the row's *)
(* own cut law is within the row's published number of the same law at the    *)
(* other pair, the ideal cut being the uniform rotation law on the cut group. *)
(* That is not independence of the reading from the secret, which the exact   *)
(* arm states and which the uniform row alone carries. It is conditional on a *)
(* coalition of fewer than two seats. And it says nothing about the full      *)
(* reveal. Each certified row is written in both of the forms the tree uses,  *)
(* once at the certificate's own spectral number and once concluded at the    *)
(* constant a text quotes, two to the minus thirty-ninth for the repeated row *)
(* and one twenty-fifth for the one-cut row.                                  *)
(*                                                                            *)
...
(* criteria are met at both Kim rows.                                         *)
(* instances/s5/tableau/s5_tableau_sampled.v records the other answer for     *)
(* s5_row_word: there the constancy an input-indistinguishability certificate *)
(* asks for is false.                                                         *)
(*                                                                            *)
...
(*                           == the same two concluded at 2^-39 and 1/25      *)
...
(*                           == the repeated row at 2^-39 and at its own      *)
