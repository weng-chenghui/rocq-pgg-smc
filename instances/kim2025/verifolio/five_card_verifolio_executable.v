...
(******************************************************************************)
(* five_card_verifolio_executable: the five-card instance at the Executable     *)
(* level                                                                      *)
(*                                                                            *)
...
(* Targeted rather than written again, which is what keeps the function a     *)
(* program names and the value its run is meant to recover one term.          *)
(*                                                                            *)
...
(* edge of a program the surface cannot write when the two levels are two     *)
(* files. The record the line builds is encoded_input_params at               *)
(* five_card_target's own function, which is the record the encoded rule      *)
(* builds from that same Targeted.                                            *)
(*                                                                            *)
(* Definitions:                                                               *)
...
From pgg_smc Require Import five_card_verifolio_algebraic.
...
(** The committed mode at the Executable level: the run argument is the pair
    ...
    out again. *)
Definition five_card_committed_executable : Verifolio Executable :=
...
Lemma five_card_committed_executable_paramsE :
  projT2 (verifolio_at five_card_committed_executable)
  = five_card_params.
